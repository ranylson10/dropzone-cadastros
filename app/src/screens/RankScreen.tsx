    ...
    <ScrollView style={styles.page} contentContainerStyle={styles.content}>
      ...
      <View style={styles.tabs}>
        ...
        >
          <Ionicons
            name="shield-outline"
            size={16}
            color={tab === "teams" ? colors.onBrand : colors.ink}
          />
          <Text
            style={[styles.tabText, tab === "teams" && styles.tabTextActive]}
          >
            Equipes
          </Text>
        </TouchableOpacity>
        ...
        >
          <Ionicons
            name="person-outline"
            size={16}
            color={tab === "players" ? colors.onBrand : colors.ink}
          />
          <Text
            style={[styles.tabText, tab === "players" && styles.tabTextActive]}
          >
            MVP
          </Text>
        </TouchableOpacity>
      ...
      {loading ? (
        <View style={styles.loading}>
          ...
          <Text style={styles.loadingText}>Carregando rank...</Text>
        </View>
      ) : null}
      ...
      <View style={styles.list}>
        {tab === "teams"
          ? teams.slice(0, 50).map((team, index) => (
              <TouchableOpacity
                key={String(team.key || team.nome || index)}
                style={styles.row}
              ...
              >
                ...
                <View style={styles.copy}>
                  ...
                  <Text style={styles.meta} numberOfLines={1}>
                    {Number(team.quedas || 0)} partidas ·{" "}
                    {Number(team.booyahs || 0)} booyahs ·{" "}
                    {Number(team.abates || 0)} abates
                  </Text>
                </View>
                <View style={styles.metric}>
                  <Text style={styles.metricValue}>
                    {Number(team.pontos || 0)}
                  </Text>
          ...
          : players.slice(0, 50).map((player, index) => (
              <View
                key={String(player.key || player.nick || index)}
                style={styles.row}
              >
                ...
                <View style={styles.copy}>
                  ...
                  <Text style={styles.teamName} numberOfLines={1}>
                    {player.equipe_tag ? `${player.equipe_tag} · ` : ""}
                    {player.equipe_nome || "Equipe não informada"}
                  </Text>
                  <Text style={styles.meta} numberOfLines={1}>
                    {Number(player.quedas || 0)} partidas · K.D{" "}
                    {kdLabel(player.abates, player.quedas)}
                  </Text>
                </View>
                <View style={[styles.metric, styles.killMetric]}>
                  <Text style={[styles.metricValue, styles.killValue]}>
                    {Number(player.abates || 0)}
                  </Text>
                  <Text style={[styles.metricLabel, styles.killLabel]}>
                    KILLS
                  </Text>
                </View>
              </View>
      ...
      {!loading && !teams.length && !players.length ? (
        <Text style={styles.empty}>Ranking ainda sem dados.</Text>
      ) : null}
    </ScrollView>
  );
}
