...
export function TeamRosterScreen({
  ...
  const openTeam = useCallback(
    async (team: TeamItem, quiet = false) => {
      ...
      >
        <View style={styles.heading}>
          ...
          <Text style={styles.description}>
            Gerencie elenco, lines, staff e campeonatos em uma experiência feita
            para o app.
          </Text>
        </View>
        ...
        {!loading && !teams.length ? (
          <View style={styles.emptyCard}>
            <View style={styles.emptyIcon}>
              <Ionicons name="shield-outline" size={30} color={colors.brand} />
            </View>
            <Text style={styles.emptyTitle}>
              Você ainda não controla uma equipe
            </Text>
            <Text style={styles.emptyText}>
              Quando criar uma equipe ou aceitar um convite de staff, ela
              aparecerá aqui.
            </Text>
            <TouchableOpacity
            ...
            >
              <Ionicons name="mail-outline" size={18} color={colors.onBrand} />
            ...
            >
              ...
              <View style={styles.teamCopy}>
                ...
                <Text style={styles.teamMeta} numberOfLines={1}>
                  {team.tag || `@${team.username || "equipe"}`}
                </Text>
                <View style={styles.roleBadge}>
                  <Text style={styles.roleText}>
                    {team.papel === "dono" ? "Dono" : "Staff"}
                  </Text>
                </View>
              </View>
  ...
  return (
    <ScrollView
      ...
      refreshControl={
        <RefreshControl
          refreshing={refreshing}
          onRefresh={() => selected && void openTeam(selected, true)}
          tintColor={colors.brand}
        />
      }
    >
      <View style={styles.teamHeader}>
        <TouchableOpacity
          ...
          onPress={() => onNavigate("team_directory")}
        >
        ...
        <View style={styles.teamHeaderCopy}>
          <Text style={styles.teamHeaderName} numberOfLines={1}>
            {selected?.nome || "Equipe"}
          </Text>
          <Text style={styles.teamHeaderMeta}>
            {selected?.tag || `@${selected?.username || "equipe"}`} ·{" "}
            {selected?.papel === "dono" ? "Dono" : "Staff"}
          </Text>
        </View>
      </View>
      ...
      >
        {(
          ...
          ] as Array<[TeamSection, any, string]>
        ).map(([id, icon, label]) => (
          <TouchableOpacity
            key={id}
            style={[
          ...
          >
            <Ionicons
              name={icon}
              size={19}
              color={section === id ? colors.onBrand : colors.muted}
            />
            <Text
              style={[
                styles.sectionLabel,
                section === id && styles.sectionLabelActive,
              ]}
            >
              {label}
            </Text>
        ...
        <>
          ...
          {(overview.issues || []).map((issue, index) => (
            <View
              ...
              style={[
                styles.issueCard,
                issue.level === "attention" && styles.issueAttention,
              ]}
            >
              <Ionicons
                name={
                ...
                size={22}
                color={
                  issue.level === "attention" ? colors.warning : colors.brand
                }
              />
              <View style={styles.issueCopy}>
                <Text style={styles.issueTitle}>{issue.title}</Text>
                <Text style={styles.issueText}>{issue.detail}</Text>
              </View>
            </View>
          ))}
          <View style={styles.quickActions}>
            {selected?.permissoes?.pode_editar ? (
              <QuickAction
            ...
            <QuickAction
              ...
              onPress={() => onNavigate("invites")}
            />
            {selected?.permissoes?.pode_escalar ? (
              <QuickAction
      ...
      {!loading && detail && section === "elenco" && selected ? (
        <TeamPlayersPanel
          teamId={selected.id}
          ...
          canInvite={
            selected.papel === "dono" ||
            Boolean(selected.permissoes?.pode_gerar_token)
          }
        />
      ) : null}
      ...
      {!loading && detail && section === "lines" ? (
        <View style={styles.sectionContent}>
          <View style={styles.sectionTitleRow}>
            <View>
              ...
              <Text style={styles.blockSubtitle}>
                {counts.lines} time(s) da equipe
              </Text>
            </View>
            {selected?.permissoes?.pode_editar ? (
              <TouchableOpacity
                style={styles.smallAction}
                onPress={() => setCreatingLine((value) => !value)}
              >
                <Ionicons name="add" size={19} color={colors.onBrand} />
                <Text style={styles.smallActionText}>Nova</Text>
              </TouchableOpacity>
            ) : null}
          </View>
          {creatingLine ? (
            <View style={styles.inlineForm}>
              ...
              <TouchableOpacity
                style={[
                  styles.saveButton,
                  (!lineName.trim() || saving) && styles.buttonDisabled,
                ]}
                disabled={!lineName.trim() || saving}
                onPress={() => void createLine()}
              >
                {saving ? (
          ...
          {!counts.lines ? (
            <EntityList empty="Nenhuma line criada." items={[]} kind="line" />
          ) : (
            <View style={styles.entityList}>
              {(overview.lines || []).map((line: any, index: number) => (
                <TouchableOpacity
                  key={String(line.id || index)}
                  style={styles.entityRow}
                  onPress={() =>
                    selected && onManageLine?.(selected.id, String(line.id))
                  }
                >
                  {line.logo_url ? (
                    <Image
                      source={{ uri: externalUrl(line.logo_url) }}
                      style={styles.entityImage}
                    />
                  ) : (
                    <View style={[styles.entityImage, styles.entityFallback]}>
                      <Ionicons
                        name="layers-outline"
                        size={21}
                        color={colors.ink}
                  ...
                  <View style={styles.entityCopy}>
                    ...
                    <Text style={styles.entitySubtitle}>
                      {line.tag || "Toque para gerenciar elenco e escalações"}
                    </Text>
                  </View>
                  <Ionicons
      ...
      {!loading && detail && section === "staff" && selected ? (
        <TeamStaffPanel
          teamId={selected.id}
          accessToken={token}
          isOwner={selected.papel === "dono"}
        />
      ) : null}
      {!loading && detail && section === "eventos" ? (
        <EntityList
          empty="A equipe não está em campeonatos ativos."
          items={overview.activeRegistrations || []}
          kind="event"
        />
      ) : null}
    </ScrollView>
...
function Feedback({ text, error = false }: { text: string; error?: boolean }) {
  return (
    <View style={[styles.feedback, error && styles.feedbackError]}>
      <Text style={[styles.feedbackText, error && styles.feedbackErrorText]}>
        {text}
      </Text>
    </View>
  );
  ...
  if (!items.length)
    return (
      <View style={styles.emptyCompact}>
        ...
        <Text style={styles.emptyCompactText}>{empty}</Text>
      </View>
    );
  return (
    <View style={styles.entityList}>
      {items.map((item, index) => {
        ...
        return (
          <View
            key={String(item.id || source.id || `${kind}-${index}`)}
            style={styles.entityRow}
          >
            {image ? (
              <Image
                source={{ uri: externalUrl(image) }}
                style={styles.entityImage}
                resizeMode="cover"
              />
            ) : (
              <View style={[styles.entityImage, styles.entityFallback]}>
                <Text style={styles.entityInitial}>
...
const styles = StyleSheet.create({
  ...
  feedbackText: { color: colors.success, fontWeight: "800" },
  ...
  teamLogo: { backgroundColor: colors.brandDark, borderRadius: 8 },
  teamLogoFallback: { alignItems: "center", justifyContent: "center" },
  teamLogoText: { color: colors.ink, fontSize: 18, fontWeight: "900" },
  ...
  entityInitial: { color: colors.ink, fontSize: 18, fontWeight: "900" },
