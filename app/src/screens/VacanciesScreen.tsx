...
export function VacanciesScreen({
  ...
  useEffect(() => {
    ...
    const cartPromise = mobileApi
      .commerceCart(accessToken)
      .then((payload) => payload.items.map(mobileCommerceFromApi));
    const wishlistPromise = mobileApi
      .commerceWishlist(accessToken)
      .then((payload) => payload.items.map(mobileCommerceFromApi));
    Promise.all([
      cartPromise.catch(() => []),
      wishlistPromise.catch(() => []),
  ...
  return (
    <ScrollView style={styles.page} contentContainerStyle={styles.content}>
      ...
      <View style={styles.toolbar}>
        ...
        {profileType === "produtora" ? (
          <TouchableOpacity
            ...
            onPress={() => onNavigate("producer_overview")}
          >
      ...
      {!loading && visibleVacancies.length === 0 ? (
        <Text style={styles.empty}>Nenhum campeonato neste filtro.</Text>
      ) : null}
        ...
        {visibleVacancies.map((item) => {
          ...
          return (
            <TouchableOpacity
              ...
              style={styles.row}
              activeOpacity={0.82}
              onPress={() => onSelectChampionship?.(championship)}
            >
              ...
              <View style={styles.rowCopy}>
                <View style={styles.rowTitleLine}>
                  <Text style={styles.type}>
                    {String(item.tipo || "Campeonato").toUpperCase()}
                  </Text>
                ...
                <Text style={styles.meta} numberOfLines={1}>
                  {dateLabel(item)} · {money(item.valor_inscricao)} ·{" "}
                  {Number(item.vagas_livres || 0)} vagas
                </Text>
              </View>
              <TouchableOpacity
                hitSlop={8}
                ...
                onPress={async () => {
                  ...
                  try {
                    const payload = await mobileApi.toggleCommerceWishlist(
                      championship.id,
                      accessToken,
                    );
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  ...
                  try {
                    const payload = await mobileApi.addCommerceCart(
                      championship.id,
                      1,
                      accessToken,
                    );
...
const styles = StyleSheet.create({
  ...
  logo: { width: 44, height: 44, backgroundColor: colors.surfaceRaised, borderRadius: 8 },
