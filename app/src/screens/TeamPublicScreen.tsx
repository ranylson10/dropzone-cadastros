...
export function TeamPublicScreen({
  selectedTeamId,
  ...
  useEffect(() => {
    if (!selectedTeamId || !auth.session?.access_token) {
      setCanManage(false);
      return;
    }
    mobileApi
      .teams(auth.session.access_token)
      .then((result: any) =>
        setCanManage(
          (result.items || []).some(
            (item: any) =>
              String(item.id) === String(selectedTeamId) &&
              (item.papel === "dono" || item.permissoes?.pode_editar),
          ),
        ),
      )
  ...
  const championships = useMemo(() => {
    ...
    for (const line of lines) {
      for (const championship of line.campeonatos || []) {
        if (!map.has(String(championship.campeonato_id)))
          map.set(String(championship.campeonato_id), {
            ...championship,
            line_nome: line.nome,
          });
      }
    ...
    <ScrollView style={styles.page} contentContainerStyle={styles.content}>
      <DirectoryHero
        ...
        description={[
          ...
          team.localidade ||
            [team.cidade, team.estado, team.pais].filter(Boolean).join(" · "),
        ]
          .filter(Boolean)
          .join(" · ")}
        compact
        ...
        <View style={styles.identity}>
          ...
          <View style={styles.identityCopy}>
            ...
            <Text style={styles.identityMeta}>
              {team.tag || "Organização competitiva"}
            </Text>
          </View>
        </View>
      ) : null}
      ...
      {canManage ? (
        <TouchableOpacity
          ...
          onPress={() => selectedTeamId && onManageTeam?.(selectedTeamId)}
        >
          <Ionicons name="settings-outline" size={17} color={colors.onBrand} />
          <Text style={styles.manageButtonText}>
            Editar equipe, lines e elenco
          </Text>
        </TouchableOpacity>
      ) : null}
        ...
        <View style={styles.list}>
          {lines.map((line: any, index: number) => (
            <TouchableOpacity
              ...
              onPress={() =>
                setSelectedLineId(
                  selectedLineId === String(line.id) ? null : String(line.id),
                )
              }
            >
              <View style={styles.row}>
                ...
                <View style={styles.copy}>
                  ...
                  <Text style={styles.meta}>
                    {(line.jogadores || []).length} jogadores ·{" "}
                    {(line.campeonatos || []).length} campeonatos
                  </Text>
                </View>
                <Ionicons
                  name={
                ...
                <View style={styles.lineDetails}>
                  ...
                  {(line.jogadores || []).map(
                    (player: any, playerIndex: number) => (
                      <TouchableOpacity
                        key={String(player.id || playerIndex)}
                        style={styles.detailRow}
                        onPress={() =>
                          player.jogador_id &&
                          onSelectPlayer?.(String(player.jogador_id))
                        }
                      >
                        <Ionicons
                          name="person-outline"
                        ...
                        <Text style={styles.detailText}>
                          {player.nick || "Jogador"}
                          {player.funcao ? ` · ${player.funcao}` : ""}
                        </Text>
                      </TouchableOpacity>
                    ),
                  )}
                  {!(line.jogadores || []).length ? (
                    <Text style={styles.detailEmpty}>
                      Nenhum jogador nesta line.
                    </Text>
                  ) : null}
                  <Text style={styles.detailTitle}>EVENTOS E AGENDA</Text>
                  {(line.campeonatos || []).map(
                    (event: any, eventIndex: number) => (
                      <TouchableOpacity
                        key={String(event.campeonato_id || eventIndex)}
                        style={styles.detailRow}
                        onPress={() =>
                          onSelectChampionship?.({
                            id: String(event.campeonato_id),
                            name: event.nome || "Campeonato",
                            mode: event.tipo || "competitivo",
                  ...
                  {!(line.campeonatos || []).length ? (
                    <Text style={styles.detailEmpty}>Nenhum evento ativo.</Text>
                  ) : null}
                </View>
              ) : null}
            </TouchableOpacity>
          ...
          {!lines.length ? (
            <Text style={styles.empty}>Nenhuma line pública.</Text>
          ) : null}
        </View>
      ) : null}
        ...
        <View style={styles.list}>
          {players.map((player: any, index: number) => (
            <TouchableOpacity
              ...
              style={styles.row}
              onPress={() =>
                player.jogador_id && onSelectPlayer?.(String(player.jogador_id))
              }
            >
              {player.foto_url ? (
                <Image
          ...
          {!players.length ? (
            <Text style={styles.empty}>Nenhum jogador público.</Text>
          ) : null}
        </View>
      ) : null}
      ...
      {!loading && tab === "championships" ? (
        <View style={styles.list}>
          {championships.map((championship: any, index: number) => (
            ...
            >
              {championship.logo_url ? (
                <Image
                  source={{ uri: externalUrl(championship.logo_url) }}
                  style={styles.logo}
                  resizeMode="contain"
                />
              ) : (
              ...
              <View style={styles.copy}>
                ...
                <Text style={styles.meta}>
                  {championship.line_nome || "Participação da equipe"}
                </Text>
              </View>
          ...
          {!championships.length ? (
            <Text style={styles.empty}>Nenhum campeonato público.</Text>
          ) : null}
        </View>
      ) : null}
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  teamLogo: { width: 48, height: 48, backgroundColor: colors.surfaceRaised, borderRadius: 7 },
  identityCopy: { flex: 1 },
  identityName: {
    color: colors.ink,
    fontSize: 13,
  ...
  logo: { width: 40, height: 40, backgroundColor: colors.surfaceRaised, borderRadius: 7 },
  ...
  lineBlock: { backgroundColor: colors.surface, borderRadius: 10, overflow: "hidden" },
  lineDetails: {
    padding: 10,
    gap: 5,
