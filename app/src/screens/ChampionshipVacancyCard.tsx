...
export function ChampionshipVacancyCard(props: {
  ...
  return <View style={styles.card}>
    <TouchableOpacity activeOpacity={.9} onPress={props.onOpen} style={styles.media}>
      {item.banner_url ? <Image source={{ uri: item.banner_url }} style={styles.banner} /> : <View style={styles.bannerFallback}><Ionicons name="trophy-outline" size={34} color="#fff" /></View>}
      ...
      <TouchableOpacity accessibilityLabel="Lista de desejos" style={styles.heart} onPress={props.onWishlist}><Ionicons name={props.favorite ? 'heart' : 'heart-outline'} size={19} color="#fff" /></TouchableOpacity>
    </TouchableOpacity>
    <View style={styles.body}>
      ...
      <View style={styles.actions}>
        ...
        <TouchableOpacity accessibilityLabel={props.inCart ? 'Remover do carrinho' : 'Adicionar ao carrinho'} style={[styles.cart, props.inCart && styles.cartActive]} onPress={props.onCart}><Ionicons name={props.inCart ? 'cart' : 'cart-outline'} size={18} color="#fff" /></TouchableOpacity>
      </View>
    </View>
  </View>
}
...
const styles=StyleSheet.create({
  card:{backgroundColor:'#fff',borderRadius:10,borderWidth:1,borderColor:'#ddd7cd',overflow:'hidden'},
  ...
  badge:{position:'absolute',left:8,top:8,paddingHorizontal:7,paddingVertical:4,borderRadius:6,backgroundColor:'rgba(255,64,88,.94)'},
  badgeText:{color:'#fff',fontSize:7,fontWeight:'900'},
  live:{position:'absolute',left:8,bottom:8,paddingHorizontal:7,paddingVertical:4,borderRadius:6,backgroundColor:'rgba(13,22,36,.9)'},
  liveText:{color:'#fff',fontSize:7,fontWeight:'900'},
  heart:{position:'absolute',right:8,top:8,width:34,height:34,borderRadius:10,alignItems:'center',justifyContent:'center',backgroundColor:'rgba(13,20,30,.68)'},
  body:{padding:10,gap:8},
  titleRow:{flexDirection:'row',gap:8,alignItems:'center'},
  logo:{width:38,height:38,borderRadius:8,backgroundColor:'#f5f1ea'},
  copy:{flex:1},
  ...
  metric:{flex:1,minWidth:0,paddingVertical:6,paddingHorizontal:6,borderRadius:7,backgroundColor:'#f4f1eb'},
  ...
  progress:{height:5,borderRadius:3,backgroundColor:'#e3ddd4',overflow:'hidden'},
  progressFill:{height:'100%',backgroundColor:colors.brand},
  actions:{flexDirection:'row',alignItems:'center',gap:6},
  open:{minHeight:38,flex:1,flexDirection:'row',alignItems:'center',justifyContent:'space-between',paddingHorizontal:10,borderRadius:8,backgroundColor:'#fff4f5'},
  openText:{color:colors.brand,fontSize:9,fontWeight:'900',textTransform:'uppercase'},
  cart:{width:42,minHeight:38,flexDirection:'row',alignItems:'center',justifyContent:'center',borderRadius:8,backgroundColor:colors.brandDark},
  cartActive:{backgroundColor:'#28384f'},
})
