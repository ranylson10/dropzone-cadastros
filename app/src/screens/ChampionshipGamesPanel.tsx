...
export function ChampionshipGamesPanel({ championshipId, phases, groups, token, onChanged }: Props) {
  ...
  }, [phaseId, phaseRounds, phaseGroups])
  ...
  function buildGameBody() {
    return {
      ...
      grupos_ids: gameGroupIds, status: selectedGame?.status || 'agendado',
    }
  }
  ...
  function resetGameForm() { setEditingGameId(''); setGameName(''); setGameDate(''); setGameTime(''); setGameMatches('4'); setGameRoundId(''); setGameGroupIds([]); setGameMapCodes([]) }
  ...
  return <View style={styles.root}>
    ...
    <View style={styles.chips}>{phases.map((phase:any)=><TouchableOpacity key={phase.id} style={[styles.chip,phaseId===String(phase.id)&&styles.chipActive]} onPress={()=>setPhaseId(String(phase.id))}><Text style={[styles.chipText,phaseId===String(phase.id)&&styles.chipTextActive]}>{phase.nome}</Text></TouchableOpacity>)}</View>
