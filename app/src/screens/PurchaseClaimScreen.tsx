...
import {
  PaymentMethod,
  paymentMethodLabel,
  VacancyPaymentResult,
} from "@/lib/payments";
import {
  clearPendingVacancyPurchase,
  getPendingVacancyPurchase,
} from "@/lib/purchase-flow";
import { ActionCard, ScreenShell } from "@/screens/components";
import { colors, radius, spacing, typography } from "@/theme/tokens";
import { ScreenProps } from "@/types/dropzone";

export function PurchaseClaimScreen({
  onBack,
  onNavigate,
  ...
  const [restoringPurchase, setRestoringPurchase] =
    useState(!selectedChampionship);
  const checkoutUrl =
    payment?.payment?.paypal_approval_url ||
  ...
  useEffect(() => {
    ...
    void getPendingVacancyPurchase().then((pending) => {
      ...
      if (pending) {
        ...
        setFeedback(
          "Compra recuperada do carrinho. Verifique o pagamento para concluir a inscrição no app.",
        );
      }
  ...
  async function startPayment() {
    ...
    try {
      const response = await mobileApi.createVacancyPayment(
        { campeonato_id: championship.id, method },
        token,
      );
      setPayment(response);
      ...
      setClaimResult(null);
      const url =
        response.payment?.paypal_approval_url || response.payment?.invoice_url;
  ...
  async function refreshClaimContext(nextTeamId?: string) {
    ...
    setFeedback("");
    try {
      const result = await mobileApi.vacancyClaimContext(
        purchaseToken,
        nextTeamId || teamId || null,
        token,
      );
      setClaimContext(result);
      const resolvedTeamId = String(
        nextTeamId ||
          result.equipe_selecionada_id ||
          result.equipes?.[0]?.id ||
          "",
      );
      ...
      setSlotId("");
      if (result.consumido)
        setFeedback("Esta compra já foi utilizada e a vaga está confirmada.");
      else if (result.liberado)
        setFeedback("Pagamento confirmado. Escolha equipe, line e slot.");
      else
        setFeedback(
          "Pagamento ainda não confirmado. Atualize novamente após a confirmação.",
        );
    } catch (err: any) {
    ...
    return (
      <ScreenShell eyebrow="Compra" title="Recuperando compra" onBack={onBack}>
        <View style={styles.restoreBox}>
          ...
          <Text style={styles.restoreText}>Sincronizando checkout...</Text>
        </View>
      </ScreenShell>
    );
  }
    ...
    return (
      <ScreenShell eyebrow="Compra" title="Escolha uma vaga" onBack={onBack}>
        <ActionCard
          ...
          description="Volte para a vitrine e escolha uma vaga."
          cta="Ver vagas"
  ...
  const teams = Array.isArray(claimContext?.equipes)
    ? claimContext.equipes
    : [];
  const lines = Array.isArray(claimContext?.lines) ? claimContext.lines : [];
  const slots = Array.isArray(claimContext?.slots_livres)
    ? claimContext.slots_livres
    : [];
  const liberated = Boolean(claimContext?.liberado);
  const consumed = Boolean(claimContext?.consumido);
  const canFinish =
    liberated && teamId && slotId && Boolean(lineId || newLineName.trim());
    ...
    >
      <View style={styles.hero}>
        {championship.bannerUrl ? (
          <Image
            source={{ uri: championship.bannerUrl }}
        ...
        <View style={styles.heroContent}>
          {championship.logoUrl ? (
            <Image source={{ uri: championship.logoUrl }} style={styles.logo} />
          ) : null}
          ...
          {(["pix", "cartao", "paypal"] as PaymentMethod[]).map((item) => (
            ...
            >
              <Text
                style={[
                  styles.methodText,
                  method === item && styles.methodTextActive,
                ]}
              >
        ...
        >
          {loading ? (
            ...
            <Text style={styles.primaryText}>
              {payment ? "Atualizar cobrança" : "Gerar pagamento"}
            </Text>
          )}
        </TouchableOpacity>
      </View>
      ...
      {payment ? (
        <View style={styles.result}>
          ...
          <Text style={styles.resultLine}>
            Valor:{" "}
            {(payment.compra.valor_centavos / 100).toLocaleString("pt-BR", {
              style: "currency",
              currency: "BRL",
            })}
          </Text>
          {payment.payment?.pix_payload ? (
            <Text style={styles.copyBox} selectable>
              {payment.payment.pix_payload}
            </Text>
          ) : null}
          <View style={styles.resultActions}>
            {checkoutUrl ? (
              <TouchableOpacity
                ...
                onPress={() => Linking.openURL(externalUrl(checkoutUrl))}
              >
                <Text style={styles.secondaryText}>Abrir pagamento</Text>
              </TouchableOpacity>
            ) : null}
            <TouchableOpacity
              style={styles.secondaryDark}
              disabled={claimLoading}
              onPress={() => void refreshClaimContext()}
            >
              {claimLoading ? (
                <ActivityIndicator color={colors.onBrand} />
              ) : (
                <Text style={styles.secondaryDarkText}>
                  Verificar pagamento
                </Text>
              )}
            </TouchableOpacity>
      ...
      {payment && liberated && !consumed ? (
        <View style={styles.claimCard}>
          ...
          <Text style={styles.claimHint}>
            Use a equipe que você controla. Cada vaga exige uma line livre neste
            campeonato.
          </Text>
          ...
          <View style={styles.choices}>
            {teams.map((team: any) => (
              <Choice
                key={String(team.id)}
                label={team.nome || team.tag || "Equipe"}
                active={String(team.id) === teamId}
          ...
          {!teams.length ? (
            <ActionCard
              title="Nenhuma equipe controlável"
              description="Crie uma equipe ou aceite acesso de staff antes de concluir a vaga."
              cta="Abrir equipes"
          ...
          {teamId ? (
            <>
              ...
              <View style={styles.choices}>
                ...
                <Choice
                  label="Criar nova line"
                  active={!lineId && Boolean(newLineName)}
                  onPress={() => setLineId("")}
                />
              </View>
              {!lineId ? (
                <TextInput
                  ...
                  style={styles.input}
                  placeholder="Nome real da nova line"
              ...
              <Text style={styles.label}>
                SLOT · {claimContext?.grupo?.nome || "GRUPO LIBERADO"}
              </Text>
              <View style={styles.choices}>
                {slots.map((slot: any) => (
                  <Choice
                    key={String(slot.id)}
                    label={`Slot ${slot.slot_letra || slot.slot_numero}`}
                    active={String(slot.id) === slotId}
                    onPress={() => setSlotId(String(slot.id))}
                  />
                ))}
              </View>
              {!slots.length ? (
                <Text style={styles.warning}>
                  Não há slot livre no grupo liberado. Atualize o pagamento para
                  buscar a próxima vaga disponível.
                </Text>
              ) : null}

              <TouchableOpacity
                style={[styles.primary, !canFinish && styles.disabled]}
                disabled={!canFinish || claimLoading}
                onPress={() => void finishClaim()}
              >
                {claimLoading ? (
                  <ActivityIndicator color={colors.onBrand} />
                ) : (
                  <Text style={styles.primaryText}>
                    Confirmar vaga no campeonato
                  </Text>
                )}
              </TouchableOpacity>
            </>
        ...
        <View style={styles.completed}>
          ...
          <View style={styles.completedCopy}>
            ...
            <Text style={styles.completedText}>
              {claimResult?.mensagem ||
                "Sua vaga já foi vinculada a uma line no campeonato."}
            </Text>
          </View>
          <TouchableOpacity
          ...
          >
            <Text style={styles.completedButtonText}>Meus campeonatos</Text>
          </TouchableOpacity>
        </View>
      ) : null}
      ...
      <View style={styles.steps}>
        ...
        <Step
          number="2"
          text="Confirme ou escolha a equipe"
          active={liberated}
        />
        <Step number="3" text="Escolha line e slot" active={liberated} />
        <Step
          number="4"
          text="Escalone o elenco dentro do prazo"
          active={consumed}
        />
      </View>
    </ScreenShell>
...
function Step(props: { number: string; text: string; active?: boolean }) {
  return (
    <View style={styles.step}>
      <Text
        style={[styles.stepNumber, props.active && styles.stepNumberActive]}
      >
        {props.number}
      </Text>
...
const styles = StyleSheet.create({
  ...
  heroOverlay: {
    ...
    bottom: 0,
    left: 0,
    backgroundColor: "rgba(8,12,18,.58)",
  },
  heroContent: {
    flex: 1,
  ...
  resultLine: { color: colors.muted, fontWeight: "700" },
  copyBox: {
    backgroundColor: colors.surface,
    color: colors.ink,
    fontSize: typography.caption,
  ...
  stepNumberActive: { backgroundColor: colors.brand, color: colors.onBrand },
