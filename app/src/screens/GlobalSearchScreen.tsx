  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        setChampionships(
          ((champRes.announcements || []) as VacancyApiItem[])
            .filter((item) =>
              [item.nome, item.tipo, item.plataforma, item.servidor].some(
                (value) =>
                  String(value || "")
                    .toLowerCase()
                    .includes(lowered),
    ...
    >
      <View style={styles.hero}>
        ...
        <Text style={styles.heroText}>
          Encontre campeonatos, equipes e jogadores sem sair do fluxo público.
        </Text>
      </View>
      ...
      >
        {(["todos", "campeonatos", "equipes", "jogadores"] as SearchKind[]).map(
          (value) => (
            <TouchableOpacity
              key={value}
              style={[styles.filter, kind === value && styles.filterActive]}
              onPress={() => setKind(value)}
            >
              <Text
                style={[
                  styles.filterText,
                  kind === value && styles.filterTextActive,
                ]}
              >
                {value} {query.trim().length >= 2 ? counts[value] : ""}
              </Text>
            </TouchableOpacity>
          ),
        )}
      ...
      {query.trim().length < 2 ? (
        <View style={styles.empty}>
          ...
          <Text style={styles.emptyTitle}>Digite pelo menos 2 caracteres</Text>
          <Text style={styles.emptyText}>
            A busca consulta os diretórios públicos oficiais.
          </Text>
        </View>
      ) : null}
      {loading ? (
      ...
      {!loading && query.trim().length >= 2 && total === 0 && !error ? (
        <View style={styles.empty}>
          ...
          <Text style={styles.emptyText}>
            Tente outro nome, nick, tag ou ID.
          </Text>
        </View>
      ) : null}

      {showChampionships && championships.length ? (
        <Section title="CAMPEONATOS" count={championships.length}>
          {championships.map((item) => {
            ...
            return (
              <TouchableOpacity
                key={String(item.id)}
                style={styles.row}
                onPress={() => onSelectChampionship?.(toChampionshipCard(item))}
              >
                {logo ? (
                  <Image
                    source={{ uri: logo }}
                  ...
                  <Fallback icon="trophy-outline" />
                )}
                <View style={styles.copy}>
                  <Text style={styles.kicker}>
                    {String(item.tipo || "Campeonato")}
                  </Text>
                  ...
                  <Text style={styles.meta} numberOfLines={1}>
                    {[
                      item.plataforma,
                      item.servidor,
                      Number(item.vagas_livres || 0) > 0
                        ? `${Number(item.vagas_livres)} vagas`
                        : null,
                    ]
                      .filter(Boolean)
                      .join(" · ") || "Competição pública"}
                  </Text>
                </View>
                <Ionicons
            ...
            return (
              ...
              >
                {logo ? (
                  ...
                  <Fallback icon="shield-outline" />
                )}
                <View style={styles.copy}>
                  <Text style={styles.kicker}>{team.tag || "Equipe"}</Text>
                  <Text style={styles.title} numberOfLines={1}>
                    {team.nome || "Equipe"}
                  </Text>
                  <Text style={styles.meta} numberOfLines={1}>
                    {[team.username ? `@${team.username}` : null, location]
                      .filter(Boolean)
                      .join(" · ") || "Equipe competitiva"}
                  </Text>
                </View>
                <Ionicons
                  name="chevron-forward"
      ...
      {showPlayers && players.length ? (
        <Section title="JOGADORES" count={players.length}>
          {players.map((player) => {
            const avatar = externalUrl(
              player.avatar_url || player.foto_url || "",
            );
            const name =
              player.nick || player.nome || player.username || "Jogador";
            return (
              <TouchableOpacity
                key={player.id}
                style={styles.row}
              ...
              >
                {avatar ? (
                  <Image
                    source={{ uri: avatar }}
                    style={[styles.logo, styles.avatar]}
                    resizeMode="cover"
                  />
                ) : (
                  <Fallback icon="person-outline" round />
                )}
                <View style={styles.copy}>
                  <Text style={styles.kicker}>
                  ...
                  <Text style={styles.meta} numberOfLines={1}>
                    {[
                      player.id_jogo ? `ID ${player.id_jogo}` : null,
                      player.localidade,
                    ]
                      .filter(Boolean)
                      .join(" · ") || "Perfil competitivo"}
                  </Text>
                </View>
                <Ionicons
                  name="chevron-forward"
...
function Fallback({ icon, round = false }: { icon: any; round?: boolean }) {
  return (
    <View style={[styles.logo, styles.fallback, round && styles.avatar]}>
...
const styles = StyleSheet.create({
  ...
  loadingText: { color: colors.muted, fontSize: 9, fontWeight: "800" },
  error: {
  ...
  sectionCount: { color: colors.brand, fontSize: 8, fontWeight: "900" },
  list: { gap: 8 },
  row: {
    minHeight: 64,
    flexDirection: "row",
