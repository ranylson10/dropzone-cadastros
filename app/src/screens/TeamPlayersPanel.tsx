...
export function TeamPlayersPanel({
  ...
  const selectedLine = useMemo(
    () => lines.find((item) => String(item.id) === selectedLineId) || null,
  ...
  const eligibleRegistrations = useMemo(
    () =>
      registrations.filter(
        (item) =>
          !selectedLineId ||
          String(item.line_id || item.line?.id || "") === selectedLineId,
      ),
    [registrations, selectedLineId],
  );

  const loadInvites = useCallback(async () => {
    if (!accessToken || !teamId || !canInvite) {
      setInvites([]);
      return;
    }
    setLoading(true);
    setError("");
    try {
      const response = await mobileApi.teamRosterInvites(
        teamId,
        selectedLineId || null,
        accessToken,
      );
      ...
      setError(
        err?.message || "Não foi possível carregar os convites de elenco.",
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (
      ...
      !eligibleRegistrations.some(
        (item) => String(item.id) === selectedRegistrationId,
      )
    ) {
  ...
  async function createInvite() {
    ...
    try {
      ...
      await Share.share({
        message: result.texto || result.url,
        url: result.url,
      });
      await loadInvites();
    } catch (err: any) {
  ...
  async function shareInvite(item: any) {
    ...
    await Share.share({
      message: `Convite para entrar na equipe: ${url}`,
      url,
    });
  }
  ...
  function renew(item: any) {
    Alert.alert(
      "Renovar convite?",
      "A validade será estendida por mais 7 dias.",
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: () =>
            void (async () => {
              ...
              setFeedback("");
              try {
                await mobileApi.renewTeamRosterInvite(
                  teamId,
                  String(item.id),
                  accessToken,
                );
                ...
                setError(err?.message || "Não foi possível renovar o convite.");
              } finally {
                setBusy(false);
              }
            })(),
      ...
      { text: "Manter", style: "cancel" },
      {
        ...
        onPress: () =>
          void (async () => {
            ...
            setFeedback("");
            try {
              await mobileApi.cancelTeamRosterInvite(
                teamId,
                String(item.id),
                accessToken,
              );
              ...
              setError(err?.message || "Não foi possível cancelar o convite.");
            } finally {
              setBusy(false);
            }
          })(),
  ...
  return (
    <View style={styles.wrap}>
      ...
      <View style={styles.card}>
        ...
        {!players.length ? (
          <Empty text="O elenco desta equipe está vazio." />
        ) : (
          players.map((item, index) => (
            <PlayerRow
              key={String(item.id || item.jogador_id || index)}
              item={item}
            />
          ))
        )}
      ...
      {canInvite ? (
        <View style={styles.card}>
          ...
          <Text style={styles.hint}>
            O link pode adicionar o jogador à equipe, a uma line específica e,
            quando selecionado, à formação do campeonato.
          </Text>
          ...
          <View style={styles.choices}>
            ...
            {lines.map((line) => (
              <Choice
                ...
                active={String(line.id) === selectedLineId}
                onPress={() => {
          ...
          {selectedLineId && eligibleRegistrations.length ? (
            <>
              <Text style={styles.label}>CAMPEONATO (OPCIONAL)</Text>
              <View style={styles.choices}>
                <Choice
                  label="Sem campeonato"
                  active={!selectedRegistrationId}
                  onPress={() => setSelectedRegistrationId("")}
                />
                {eligibleRegistrations.map((item) => (
                  <Choice
                    ...
                    active={String(item.id) === selectedRegistrationId}
                    onPress={() => setSelectedRegistrationId(String(item.id))}
                  />
                ))}
              </View>
            </>
          ...
          >
            {busy ? (
              ...
              <>
                ...
                <Text style={styles.primaryText}>Gerar e compartilhar</Text>
              </>
            )}
          </TouchableOpacity>
          {selectedLine ? (
            <Text style={styles.destination}>
              Destino atual:{" "}
              {selectedLine.nome || selectedLine.tag || "Line selecionada"}
              {selectedRegistrationId ? " + campeonato" : ""}
            </Text>
          ) : (
            <Text style={styles.destination}>
              Destino atual: equipe, sem line específica
            </Text>
          )}
        </View>
      ) : (
        <View style={styles.readOnly}>
          ...
          <Text style={styles.readOnlyText}>
            Seu acesso permite visualizar o elenco, mas não gerar convites de
            jogador.
          </Text>
        </View>
      )}
        ...
        <View style={styles.card}>
          ...
          {loading ? (
            <View style={styles.loading}>
              ...
              <Text style={styles.hint}>Atualizando convites...</Text>
            </View>
          ) : null}
          {!loading && !invites.length ? (
            <Empty text="Nenhum convite ativo para este destino." />
          ) : (
            invites.map((item) => (
              <View key={item.id} style={styles.inviteRow}>
                ...
                <View style={styles.copy}>
                  <Text style={styles.name}>
                    {item.expired ? "Expirado" : "Convite ativo"}
                  </Text>
...
function PlayerRow({ item }: { item: any }) {
  ...
  return (
    <View style={styles.playerRow}>
      {image ? (
        <Image source={{ uri: externalUrl(image) }} style={styles.avatar} />
      ) : (
      ...
      <View style={styles.copy}>
        ...
        <Text style={styles.meta}>
          {role || "Função não informada"} · ID {id || "pendente"}
        </Text>
      </View>
    </View>
  );
...
function Empty({ text }: { text: string }) {
  return (
    <View style={styles.empty}>
      <Ionicons name="file-tray-outline" size={23} color={colors.muted} />
