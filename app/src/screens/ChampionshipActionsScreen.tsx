...
export function ChampionshipActionsScreen({
  onBack,
  onNavigate,
  selectedLineup,
}: ScreenProps) {
  if (!selectedLineup) {
    return (
      <ScreenShell
        eyebrow="Campeonato"
        title="Ações"
        description="Escolha um campeonato para abrir as ações rápidas."
        onBack={onBack}
      >
        <ActionCard
          title="Nenhum campeonato selecionado"
          description="Abra a lista de campeonatos inscritos e escolha uma line/equipe."
          cta="Meus campeonatos"
  ...
  const confirmed = Number(
    selectedLineup.jogadores_confirmados ||
      selectedLineup.jogadores?.length ||
      0,
  );
  const limit = Number(selectedLineup.limite_jogadores || 6);
  const championshipTeamId = String(selectedLineup.campeonato_equipe_id || "");
  const siteUrl = championshipTeamId
    ? apiUrl(
        `/painel?campeonatoEquipe=${encodeURIComponent(championshipTeamId)}`,
      )
    : apiUrl("/painel");
  ...
  return (
    ...
    >
      <View style={styles.metrics}>
        ...
        <MetricPill
          label="status"
          value={confirmed >= limit ? "OK" : "Pendente"}
        />
      </View>
