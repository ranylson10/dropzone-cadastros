...
export function TeamStaffPanel({
  ...
  function toggle(key: keyof Permissions) {
    setPermissions((current) => ({
      ...current,
      [key]: key === "pode_ver" ? true : !current[key],
    }));
  }
  ...
  async function invite() {
    ...
    setFeedback("");
    try {
      const response = await mobileApi.inviteTeamStaff(
        teamId,
        {
  ...
  async function savePermissions(item: any, next: Permissions) {
    ...
    setFeedback("");
    try {
      await mobileApi.updateTeamStaff(
        teamId,
        { manager_id: item.manager_id, ...next },
        accessToken,
      );
      ...
      { text: "Cancelar", style: "cancel" },
      {
        ...
        onPress: () =>
          void (async () => {
            ...
            setFeedback("");
            try {
              await mobileApi.removeTeamStaff(
                teamId,
              ...
              setError(err?.message || "Não foi possível remover o manager.");
            } finally {
              setSaving(false);
            }
          })(),
  ...
  function cancelInvite(item: any) {
    Alert.alert(
      "Cancelar convite?",
      "O convite pendente deixará de poder ser aceito.",
      [
        { text: "Manter", style: "cancel" },
        {
          ...
          onPress: () =>
            void (async () => {
              ...
              setFeedback("");
              try {
                await mobileApi.cancelTeamStaffInvite(
                  teamId,
                  String(item.id),
                  accessToken,
                );
                ...
                setError(
                  err?.message || "Não foi possível cancelar o convite.",
                );
              } finally {
                setSaving(false);
              }
  ...
  if (loading)
    return (
      <View style={styles.loading}>
        ...
        <Text style={styles.muted}>Carregando staff...</Text>
      </View>
    );
  ...
  return (
    <View style={styles.wrap}>
      ...
      {isOwner ? (
        <View style={styles.card}>
          ...
          <Text style={styles.hint}>
            Busque pelo @username ou ID público do manager. O convite chega pela
          ...
          <TextInput
            ...
            style={styles.input}
            autoCapitalize="none"
            placeholder="@username ou ID público"
          ...
          <Permission
            label="Ver equipe"
            active={permissions.pode_ver}
            locked
            onPress={() => {}}
          />
          <Permission
            label="Editar equipe e lines"
            active={permissions.pode_editar}
            onPress={() => toggle("pode_editar")}
          />
          <Permission
            label="Gerenciar escalações"
            active={permissions.pode_escalar}
            onPress={() => toggle("pode_escalar")}
          />
          <Permission
            label="Gerar tokens e convites"
            active={permissions.pode_gerar_token}
            onPress={() => toggle("pode_gerar_token")}
          />
          <TouchableOpacity
            disabled={saving || !query.trim()}
            style={[
              styles.primary,
              (saving || !query.trim()) && styles.disabled,
            ]}
            onPress={() => void invite()}
          >
            {saving ? (
              ...
              <>
                <Ionicons name="paper-plane-outline" size={17} color={colors.onBrand} />
        ...
        <View style={styles.readOnly}>
          ...
          <Text style={styles.readOnlyText}>
            Somente o dono da equipe pode convidar, remover ou alterar
            permissões da staff.
          </Text>
        </View>
      )}
      ...
      <View style={styles.card}>
        ...
        {!staff.length ? (
          <Text style={styles.empty}>Nenhum manager adicional na staff.</Text>
        ) : (
          staff.map((item) => (
            <StaffRow
              key={item.id}
              ...
              <View key={item.id} style={styles.inviteRow}>
                ...
                <View style={styles.copy}>
                  <Text style={styles.name}>
                    {item.manager?.nome || item.manager_username || "Manager"}
                  </Text>
                ...
                <TouchableOpacity
                  ...
                  disabled={saving}
                >
                  <Ionicons name="close-outline" size={19} color={colors.danger} />
                </TouchableOpacity>
              </View>
            ))
          )}
...
function StaffRow({
  ...
  useEffect(
    () => setDraft(current),
    [item.pode_ver, item.pode_editar, item.pode_escalar, item.pode_gerar_token],
  );
  ...
  return (
    <View style={styles.staffRow}>
      <View style={styles.personRow}>
        ...
        <View style={styles.copy}>
          <Text style={styles.name}>
            {item.manager?.nome || item.manager?.username || "Manager"}
          </Text>
          <Text style={styles.meta}>
            @{item.manager?.username || "manager"}
          </Text>
        </View>
        {isOwner ? (
          <TouchableOpacity
      ...
      <View style={styles.permissions}>
        <Permission
          label="Ver"
          active={draft.pode_ver}
          locked
          onPress={() => {}}
        />
        <Permission
          label="Editar"
          active={draft.pode_editar}
          disabled={!isOwner}
          onPress={() =>
            setDraft((v) => ({ ...v, pode_editar: !v.pode_editar }))
          }
        />
        <Permission
          label="Escalar"
          active={draft.pode_escalar}
          disabled={!isOwner}
          onPress={() =>
            setDraft((v) => ({ ...v, pode_escalar: !v.pode_escalar }))
          }
        />
        <Permission
          label="Tokens"
          active={draft.pode_gerar_token}
          disabled={!isOwner}
          onPress={() =>
            setDraft((v) => ({ ...v, pode_gerar_token: !v.pode_gerar_token }))
          }
        />
      </View>
      {isOwner && dirty ? (
        ...
        >
          <Text style={styles.saveSmallText}>Salvar permissões</Text>
        </TouchableOpacity>
      ) : null}
    </View>
  );
  ...
  return (
    <TouchableOpacity
      ...
      style={[
        ...
        (locked || disabled) && styles.permissionDisabled,
      ]}
    >
      <Ionicons
      ...
      />
      <Text
        style={[styles.permissionText, active && styles.permissionTextActive]}
      >
        {label}
      </Text>
    </TouchableOpacity>
  ...
  return (
    <View style={[styles.avatar, styles.avatarFallback]}>
      <Text style={styles.avatarText}>
        {String(source?.nome || source?.username || "M")
          .slice(0, 1)
          .toUpperCase()}
      </Text>
    </View>
...
const styles = StyleSheet.create({
  ...
  message: { minHeight: 64, paddingTop: 10, textAlignVertical: "top" },
  daysRow: {
    flexDirection: "row",
    alignItems: "center",
  ...
  meta: { marginTop: 2, color: colors.muted, fontSize: 8 },
  ...
  permissionActive: { backgroundColor: "rgba(101,185,130,.13)", borderColor: colors.success },
  permissionDisabled: { opacity: 0.7 },
  permissionText: {
    color: colors.muted,
    fontSize: 7,
    fontWeight: "900",
  ...
  permissionTextActive: { color: colors.success },
  saveSmall: {
