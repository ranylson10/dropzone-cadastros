...
export function LineupScreen({
  ...
  const visibleLineups = useMemo(() => {
    ...
    const focused = lineups.filter(
      (item) => String(item.campeonato_equipe_id || "") === selectedId,
    );
  ...
  const totals = useMemo(
    () =>
      visibleLineups.reduce(
        (acc, lineup) => {
          const limit = Number(lineup.limite_jogadores || 6);
          const confirmed = Number(
            lineup.jogadores_confirmados || lineup.jogadores?.length || 0,
          );
  ...
  async function generateInvite(lineup: any) {
    ...
    setFeedback("");
    try {
      const created = await mobileApi.createLineupInvite(
        participationId,
        accessToken,
      );
      setFeedback("Novo convite criado. O link anterior foi encerrado.");
      await Share.share({
        message: created.texto || created.public_url,
        url: created.public_url,
      });
      await load();
    } catch (err: any) {
      setError(
        err?.message || "Não foi possível gerar o convite de escalação.",
      );
    } finally {
      setBusyId(null);
    }
  ...
  function revokeInvite(lineup: any) {
    ...
    Alert.alert(
      "Encerrar convite?",
      "O link atual deixará de aceitar novos jogadores.",
      [
          ...
          onPress: () =>
            void (async () => {
              ...
              try {
                ...
                setError(
                  err?.message || "Não foi possível encerrar o convite.",
                );
              } finally {
                setBusyId(null);
              }
  ...
  function removePlayer(player: any) {
    ...
    Alert.alert(
      ...
      `${player.nick || "Jogador"} será removido desta formação.`,
      [
  ...
  return (
    <ScreenShell
      eyebrow="Campeonato"
      title={selectedLineup?.campeonato_nome || "Escalação"}
      onBack={onBack}
    >
      <View style={styles.hero}>
        <Text style={styles.heroKicker}>
          {selectedLineup ? "Formação selecionada" : "Resumo do elenco"}
        </Text>
        <Text style={styles.heroTitle}>
          {totals.confirmed}/{totals.limit || 0}
        </Text>
        <Text style={styles.heroText}>
          {totals.open} vaga(s) ainda abertas nas formações exibidas.
        </Text>
      </View>
      ...
      {loading ? (
        <View style={styles.loading}>
          ...
          <Text style={styles.muted}>Carregando escalação...</Text>
        </View>
      ) : null}
      ...
      {!loading && visibleLineups.length === 0 ? (
        <ActionCard
          title="Nenhuma line inscrita"
          description="Entre em um campeonato com vaga. Depois a escalação aparece aqui."
          cta="Buscar vagas"
      ...
      {visibleLineups.map((lineup: any) => {
        const id = String(
          lineup.campeonato_equipe_id || lineup.campeonato_nome || "",
        );
        const limit = Number(lineup.limite_jogadores || 6);
        const players = Array.isArray(lineup.jogadores) ? lineup.jogadores : [];
        const confirmed = Number(
          lineup.jogadores_confirmados || players.length || 0,
        );
        const free = Math.max(
          0,
          Number(lineup.vagas_disponiveis ?? limit - confirmed),
        );
        const progress = Math.max(
          4,
          Math.min(100, (confirmed / Math.max(1, limit)) * 100),
        );
          ...
          <View key={id} style={styles.card}>
            <View style={styles.cardHead}>
              <View style={styles.slotBox}>
                <Text style={styles.slotValue}>{confirmed}</Text>
                <Text style={styles.slotLabel}>/{limit}</Text>
              </View>
              <View style={styles.cardTitleWrap}>
                <Text style={styles.cardTitle} numberOfLines={1}>
                  {lineup.campeonato_nome || "Campeonato"}
                </Text>
                <Text style={styles.cardMeta} numberOfLines={2}>
                  {lineupSubtitle(lineup)}
                </Text>
              </View>
            </View>
            ...
            {!players.length ? (
              <Text style={styles.empty}>
                Nenhum jogador confirmou participação nesta escalação.
              </Text>
            ) : (
              players.map((player: any) => (
                <View key={String(player.id)} style={styles.playerRow}>
                  {player.foto_url ? (
                    <Image
                  ...
                  <TouchableOpacity
                    disabled={busyId === String(player.id)}
              ...
              <View style={styles.tokenBox}>
                <View style={styles.tokenCopy}>
                  ...
                  <Text style={styles.tokenMeta}>
                    {lineup.link_expira_em
                      ? `Validade: ${new Date(lineup.link_expira_em).toLocaleString("pt-BR")}`
                      : "Validade definida pelas regras do campeonato"}
                  </Text>
                </View>
                <TouchableOpacity
                  style={styles.iconButton}
                  onPress={() => void shareInvite(lineup)}
                >
                  <Ionicons
                ...
                <TouchableOpacity
                  accessibilityLabel="Abrir link de escalação"
                  style={styles.iconButton}
                  onPress={() => void openInvite(lineup)}
                >
                  <Ionicons name="open-outline" size={18} color={colors.ink} />
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.iconDanger}
                  disabled={busyId === String(lineup.link_id)}
                  onPress={() => revokeInvite(lineup)}
                >
                  <Ionicons
            ...
            <TouchableOpacity
              ...
              onPress={() => void generateInvite(lineup)}
              disabled={busyId === id || free <= 0}
            >
              {busyId === id ? (
                ...
                <Text style={styles.primaryText}>
                  {hasToken
                    ? "Gerar novo convite"
                    : "Gerar convite de escalação"}
                </Text>
              )}
            </TouchableOpacity>
            {free <= 0 ? (
              <Text style={styles.complete}>
                Formação completa. Remova um jogador antes de gerar nova vaga.
              </Text>
            ) : null}
          </View>
        );
...
const styles = StyleSheet.create({
  ...
  slotValue: { color: colors.ink, fontSize: 20, fontWeight: "900" },
  slotLabel: { color: colors.muted, fontSize: 9, fontWeight: "900" },
  ...
  progress: { height: 6, overflow: "hidden", backgroundColor: colors.surfaceRaised },
