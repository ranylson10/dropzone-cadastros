...
begin
  ...
  loop
    ...
    v_logo_url := nullif(left(trim(coalesce(v_item->>'logo_url', '')), 2000), '');
    ...
    values (v_nome, v_tag, v_logo_url, null, null, 'ativo')
    ...
    values (
      ...
      jsonb_build_object('produtora_id', p_produtora_id, 'nome', v_nome, 'tag', v_tag, 'line_id', v_line_id, 'logo_url', v_logo_url)
    );
