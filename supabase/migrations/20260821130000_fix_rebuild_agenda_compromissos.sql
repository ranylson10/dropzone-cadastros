-- Corrige a ordem da validação: DELETE altera FOUND no PL/pgSQL, portanto o
-- jogo deve ser validado antes de limpar a projeção anterior.
...
begin
  ...
  insert into public.agenda_compromissos (jogo_id, campeonato_id, destino_tipo, destino_id, titulo, campeonato_nome, data_evento, horario_inicio, horario_fim, status, owner_auth_user_id)
  select distinct v_jogo.id, v_jogo.campeonato_id, 'equipe', ce.equipe_id, coalesce(nullif(v_jogo.nome, ''), 'Jogo'), v_campeonato.nome, v_jogo.data_jogo, v_jogo.horario, v_fim, coalesce(v_jogo.status, 'agendado'), v_campeonato.criado_por
