...
create table if not exists public.sistema_vendas_assistidas (
  ...
  referencia text,
  status text not null default 'aberta' check (status in ('aberta','checkout_iniciado','paga','expirada','cancelada')),
...
create index if not exists sistema_vendas_assistidas_compra_idx on public.sistema_vendas_assistidas(compra_vaga_id) where compra_vaga_id is not null;
...
create policy sistema_vendas_assistidas_service_all on public.sistema_vendas_assistidas for all using (
...
comment on table public.sistema_vendas_assistidas is 'Checkout público atribuído a vendedor. O vendedor não recebe nem gera PIX em nome próprio.';
commit;
