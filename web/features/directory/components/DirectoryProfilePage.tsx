...
export async function DirectoryProfilePage({ kind, id }: { kind: DirectoryKind; id: string }) {
  ...
  const reportType = { campeonatos: 'campeonato', equipes: 'equipe', jogadores: 'jogador', managers: 'manager', produtoras: 'produtora' }[kind]
      ...
      <main className="directory-profile-page compact-profile">
        ...
        <section className="directory-profile-hero compact">
          ...
          <div className="directory-profile-copy">
            <small>{profile.eyebrow || config.singular}</small><h1>{profile.name}</h1>{profile.username ? <strong>@{profile.username}</strong> : null}<p>{profile.description}</p>
            {profile.actions?.length ? (
              <div className="directory-profile-actions">
                {profile.actions.map((action) => <a key={action.href} className={`directory-profile-action ${action.variant || 'secondary'}`} href={action.href}>{action.label}</a>)}
              </div>
            ) : null}
