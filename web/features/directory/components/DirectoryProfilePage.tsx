    ...
    <>
      ...
      <main className="directory-profile-page compact-profile">
        ...
        <section className="directory-profile-hero compact">
          ...
          <div className="directory-profile-copy"><small>{profile.eyebrow || config.singular}</small><h1>{profile.name}</h1>{profile.username ? <strong>@{profile.username}</strong> : null}<p>{profile.description}</p><ReportButton targetType={reportType} targetId={id} targetName={profile.name} /></div>
