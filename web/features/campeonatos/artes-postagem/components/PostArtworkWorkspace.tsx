...
function CompactToolSection(props: { title: string; hint?: string; defaultOpen?: boolean; children: ReactNode }) {
  return <details className="post-artworks-tool-section" open={props.defaultOpen}><summary><strong>{props.title}</strong>{props.hint ? <small>{props.hint}</small> : null}<span>⌄</span></summary><div className="post-artworks-tool-section-body">{props.children}</div></details>
}
...
export function PostArtworkWorkspace({ campeonatoId, mode = 'edit', initialArtworkId }: { campeonatoId: string; mode?: 'edit' | 'generate' | 'manage' | 'library'; initialArtworkId?: string }) {
  ...
  const compactBlockTools = selectedBlock ? <div className="post-artworks-compact-tools">
    <CompactToolSection title="Bloco" hint="posição e dados" defaultOpen>
      <label>Nome<input value={selectedBlock.name} onChange={(event) => patchBlock(selectedBlock.id, { name: event.target.value })} /></label>
      <div className="post-artworks-grid2"><label>X<EditableNumberInput value={selectedBlock.x} min={-20000} max={20000} onCommit={(value) => patchBlock(selectedBlock.id, { x: value })} /></label><label>Y<EditableNumberInput value={selectedBlock.y} min={-20000} max={20000} onCommit={(value) => patchBlock(selectedBlock.id, { y: value })} /></label></div>
      {selectedBlock.type === 'mvp_general_table' ? <div className="post-artworks-grid2"><label>Do Top<EditableNumberInput value={selectedBlock.dataStart || 2} min={1} max={999} onCommit={(value) => patchBlock(selectedBlock.id, { dataStart: value, dataEnd: Math.max(value, selectedBlock.dataEnd || value) })} /></label><label>Até<EditableNumberInput value={selectedBlock.dataEnd || 10} min={selectedBlock.dataStart || 1} max={999} onCommit={(value) => patchBlock(selectedBlock.id, { dataEnd: value })} /></label></div> : null}
    </CompactToolSection>

    {(selectedTableStyle || selectedBooyahStyle || selectedQualifiedStyle || selectedMvpStyle) ? <CompactToolSection title="Estrutura" hint="tamanho e espaçamento">
      {selectedTableStyle ? <div className="post-artworks-grid2"><label>Altura da linha<EditableNumberInput value={selectedTableStyle.rowHeight} min={20} max={300} onCommit={(value) => patchTableStyle({ rowHeight: value })} /></label><label>Espaço linhas<EditableNumberInput value={selectedTableStyle.rowGap} min={0} max={100} onCommit={(value) => patchTableStyle({ rowGap: value })} /></label><label>Espaço colunas<EditableNumberInput value={selectedTableStyle.cellGap} min={0} max={100} onCommit={(value) => patchTableStyle({ cellGap: value })} /></label>{selectedColumn ? <label>Largura coluna<EditableNumberInput value={selectedColumn.width} min={30} max={1000} onCommit={(value) => patchColumn(selectedColumn.key, { width: value })} /></label> : null}</div> : null}
      {selectedMvpStyle ? <div className="post-artworks-grid2"><label>Largura do card<EditableNumberInput value={selectedMvpStyle.cardWidth} min={180} max={1600} onCommit={(value) => patchMvpStyle({ cardWidth: value })} /></label><label>Altura do card<EditableNumberInput value={selectedMvpStyle.cardHeight} min={220} max={2000} onCommit={(value) => patchMvpStyle({ cardHeight: value })} /></label><label>Espaçamento<EditableNumberInput value={selectedMvpStyle.gap} min={0} max={120} onCommit={(value) => patchMvpStyle({ gap: value })} /></label></div> : null}
      {selectedBooyahStyle ? <div className="post-artworks-grid2"><label>Largura geral<EditableNumberInput value={selectedBooyahStyle.totalWidth} min={180} max={5000} onCommit={(value) => patchBooyahStyle({ totalWidth: value })} /></label><label>Altura dos cards<EditableNumberInput value={selectedBooyahStyle.cardHeight} min={100} max={1200} onCommit={(value) => patchBooyahStyle({ cardHeight: value })} /></label><label>Espaçamento<EditableNumberInput value={selectedBooyahStyle.gap} min={0} max={120} onCommit={(value) => patchBooyahStyle({ gap: value })} /></label></div> : null}
      {selectedQualifiedStyle ? <div className="post-artworks-grid2"><label>Largura card<EditableNumberInput value={selectedQualifiedStyle.cardWidth} min={60} max={600} onCommit={(value) => patchQualifiedStyle({ cardWidth: value })} /></label><label>Altura card<EditableNumberInput value={selectedQualifiedStyle.cardHeight} min={60} max={600} onCommit={(value) => patchQualifiedStyle({ cardHeight: value })} /></label><label>Espaçamento<EditableNumberInput value={selectedQualifiedStyle.gap} min={0} max={120} onCommit={(value) => patchQualifiedStyle({ gap: value })} /></label></div> : null}
    </CompactToolSection> : null}

    {(selectedTableStyle || selectedBooyahStyle || selectedQualifiedStyle || selectedMvpStyle) ? <CompactToolSection title="Texto" hint="fonte, tamanho e cor">
      {selectedTableStyle && selectedColumn && selectedColumn.key !== 'logo' ? <><div className="post-artworks-grid2"><label>Tamanho<EditableNumberInput value={selectedColumn.fontSize} min={8} max={120} onCommit={(value) => patchColumn(selectedColumn.key, { fontSize: value })} /></label><label>Peso<EditableNumberInput value={selectedColumn.fontWeight} min={100} max={900} step={100} bigStep={100} onCommit={(value) => patchColumn(selectedColumn.key, { fontWeight: value })} /></label></div><PaletteColorField label="Cor do texto" value={selectedColumn.color} palette={currentPalette} onChange={(value) => patchColumn(selectedColumn.key, { color: value })} /><label>Alinhamento<select value={selectedColumn.align} onChange={(event) => patchColumn(selectedColumn.key, { align: event.target.value as 'left' | 'center' | 'right' })}><option value="left">Esquerda</option><option value="center">Centro</option><option value="right">Direita</option></select></label></> : null}
      {selectedMvpStyle ? <><div className="post-artworks-grid2"><label>Nome<EditableNumberInput value={selectedMvpStyle.nameFontSize} min={8} max={160} onCommit={(value) => patchMvpStyle({ nameFontSize: value })} /></label><label>Estatísticas<EditableNumberInput value={selectedMvpStyle.statsFontSize} min={8} max={140} onCommit={(value) => patchMvpStyle({ statsFontSize: value })} /></label></div><PaletteColorField label="Cor do nome" value={selectedMvpStyle.nameColor} palette={currentPalette} onChange={(value) => patchMvpStyle({ nameColor: value })} /><PaletteColorField label="Cor dos números" value={selectedMvpStyle.statsColor} palette={currentPalette} onChange={(value) => patchMvpStyle({ statsColor: value })} /></> : null}
      {selectedBooyahStyle ? <><div className="post-artworks-grid2"><label>Nome do mapa<EditableNumberInput value={selectedBooyahStyle.teamFontSize} min={8} max={120} onCommit={(value) => patchBooyahStyle({ teamFontSize: value })} /></label><label>Estatísticas<EditableNumberInput value={selectedBooyahStyle.statsFontSize} min={8} max={100} onCommit={(value) => patchBooyahStyle({ statsFontSize: value })} /></label></div><PaletteColorField label="Cor do texto" value={selectedBooyahStyle.textColor} palette={currentPalette} onChange={(value) => patchBooyahStyle({ textColor: value })} /></> : null}
      {selectedQualifiedStyle && selectedQualifiedStyle.showTitles ? <><label>Tamanho do título<EditableNumberInput value={selectedQualifiedStyle.titleFontSize} min={10} max={160} onCommit={(value) => patchQualifiedStyle({ titleFontSize: value })} /></label><PaletteColorField label="Cor do título" value={selectedQualifiedStyle.titleColor} palette={currentPalette} onChange={(value) => patchQualifiedStyle({ titleColor: value })} /></> : null}
    </CompactToolSection> : null}

    {(selectedMvpStyle || selectedBooyahStyle || selectedQualifiedStyle) ? <CompactToolSection title="Imagem" hint="tamanho e margem">
      {selectedMvpStyle ? <><label className="post-artworks-check"><input type="checkbox" checked={selectedMvpStyle.showPhoto} onChange={(event) => patchMvpStyle({ showPhoto: event.target.checked })} /> Exibir foto</label><div className="post-artworks-grid2"><label>Tamanho<EditableNumberInput value={selectedMvpStyle.imageSize} min={40} max={1000} onCommit={(value) => patchMvpStyle({ imageSize: value })} /></label><label>Margem<EditableNumberInput value={selectedMvpStyle.gap} min={0} max={120} onCommit={(value) => patchMvpStyle({ gap: value })} /></label><label>Canto<EditableNumberInput value={selectedMvpStyle.imageRadius} min={0} max={500} onCommit={(value) => patchMvpStyle({ imageRadius: value })} /></label></div></> : null}
      {selectedBooyahStyle ? <label>Tamanho da logo (%)<EditableNumberInput value={Math.round(selectedBooyahStyle.logoScale * 100)} min={15} max={80} onCommit={(value) => patchBooyahStyle({ logoScale: value / 100 })} /></label> : null}
      {selectedQualifiedStyle ? <label>Tamanho da logo (%)<EditableNumberInput value={Math.round(selectedQualifiedStyle.logoScale * 100)} min={10} max={100} onCommit={(value) => patchQualifiedStyle({ logoScale: value / 100 })} /></label> : null}
    </CompactToolSection> : null}

    {(selectedTableStyle || selectedBooyahStyle || selectedQualifiedStyle || selectedMvpStyle) ? <CompactToolSection title="Fundo" hint="cor, imagem e acabamento">
      {selectedTableStyle && selectedColumn ? <><label>Fundo da coluna<select value={selectedColumn.backgroundType} onChange={(event) => patchColumn(selectedColumn.key, { backgroundType: event.target.value as 'color' | 'image' | 'none' })}><option value="color">Cor</option><option value="image">Imagem</option><option value="none">Sem fundo</option></select></label>{selectedColumn.backgroundType === 'color' ? <PaletteColorField label="Cor do fundo" value={selectedColumn.backgroundColor} palette={currentPalette} onChange={(value) => patchColumn(selectedColumn.key, { backgroundColor: value })} /> : null}</> : null}
      {selectedMvpStyle ? <><label>Fundo do card<select value={selectedMvpStyle.backgroundType} onChange={(event) => patchMvpStyle({ backgroundType: event.target.value as 'color' | 'image' | 'none' })}><option value="color">Cor</option><option value="image">Imagem</option><option value="none">Sem fundo</option></select></label>{selectedMvpStyle.backgroundType === 'color' ? <PaletteColorField label="Cor do fundo" value={selectedMvpStyle.backgroundColor} palette={currentPalette} onChange={(value) => patchMvpStyle({ backgroundColor: value })} /> : null}</> : null}
      {selectedBooyahStyle ? <><PaletteColorField label="Fundo base" value={selectedBooyahStyle.backgroundColor} palette={currentPalette} onChange={(value) => patchBooyahStyle({ backgroundColor: value })} /><PaletteColorField label="Faixa do mapa" value={selectedBooyahStyle.accentColor} palette={currentPalette} onChange={(value) => patchBooyahStyle({ accentColor: value })} /></> : null}
      {selectedQualifiedStyle ? <><label>Fundo do card<select value={selectedQualifiedStyle.backgroundType} onChange={(event) => patchQualifiedStyle({ backgroundType: event.target.value as 'color' | 'image' | 'none' })}><option value="color">Cor</option><option value="image">Imagem</option><option value="none">Sem fundo</option></select></label>{selectedQualifiedStyle.backgroundType === 'color' ? <PaletteColorField label="Cor do fundo" value={selectedQualifiedStyle.backgroundColor} palette={currentPalette} onChange={(value) => patchQualifiedStyle({ backgroundColor: value })} /> : null}</> : null}
    </CompactToolSection> : null}
  </div> : null
        ...
        {draft ? <>
          <section className="post-artworks-controls">
            ...
            {!selectedBlock ? <>
              ...
              {compactBlockTools}
              <details className="post-artworks-advanced-tools"><summary>Ajustes avançados</summary><div className="post-artworks-advanced-tools-body">
