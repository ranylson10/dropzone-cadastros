        ...
        {draft ? <>
          <section className="post-artworks-controls">
            ...
            {!selectedBlock ? <>
              ...
              {(selectedBlock.type === 'table_day' || selectedBlock.type === 'booyahs_day' || selectedBlock.type === 'mvp_day' || selectedBlock.type === 'qualified_teams') ? <label>Jogo<select value={selectedBlock.source?.jogoId || ''} onChange={(event) => { const game = games.find((item) => item.id === event.target.value); patchBlock(selectedBlock.id, { source: { jogoId: event.target.value, jogoName: game?.nome || '' }, ...(selectedBlock.type === 'qualified_teams' ? { dataStart: 1, dataEnd: game?.mataMata && game.classificamQuantidade && game.classificamQuantidade > 0 ? game.classificamQuantidade : 0 } : {}) }) }}><option value="">Selecione o jogo</option>{games.filter((game) => selectedBlock.type !== 'qualified_teams' || (game.mataMata && game.classificamQuantidade)).map((game) => <option key={game.id} value={game.id}>{game.nome}{game.mataMata && game.classificamQuantidade ? ` · Top ${game.classificamQuantidade} passa` : ''}</option>)}</select></label> : null}
