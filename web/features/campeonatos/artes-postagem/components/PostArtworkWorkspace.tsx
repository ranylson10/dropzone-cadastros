...
function defaultBooyahStyle(): PostArtworkBooyahStyle {
  return { totalWidth: 980, cardHeight: 250, gap: 12, backgroundColor: '#15171C', accentColor: '#8FCE00', textColor: '#FFFFFF', logoScale: .42, teamFontSize: 24, mapFontSize: 18, statsFontSize: 20 }
}
...
function defaultMvpStyle(): PostArtworkMvpStyle {
  return {
    ...
    imageSize: 260, imageRadius: 18,
    photoX: 40, photoY: 0, photoWidth: 340, photoHeight: 390, photoFade: 'none', photoFadeStrength: 55,
    infoX: 20, infoY: 390, infoWidth: 380, infoHeight: 150,
    nameColor: '#ffffff', nameFontSize: 38, nameFontWeight: 900,
...
function createMvpBlock(type: 'mvp_general' | 'mvp_day', index: number, game?: GameOption, layoutMode?: PostArtworkMvpStyle['layoutMode']): PostArtworkBlock {
  const style = { ...defaultMvpStyle(), ...(layoutMode ? { layoutMode } : {}) }
  const isTop1 = type === 'mvp_general' && style.layoutMode === 'top1_only'
  const isRanking = type === 'mvp_general' && style.layoutMode === 'ranking_only'
  return {
    id: uid(type === 'mvp_day' ? 'mvp-day' : isTop1 ? 'mvp-top1' : isRanking ? 'mvp-ranking' : 'mvp-general'),
    type,
    name: type === 'mvp_day' ? `MVP do Jogo ${index + 1}` : isTop1 ? `MVP Geral · Top 1 ${index + 1}` : isRanking ? `MVP Geral · Top 2+ ${index + 1}` : `MVP Geral ${index + 1}`,
    x: 60, y: 160,
    width: isRanking ? style.tableWidth : style.cardWidth,
    visible: true,
    dataStart: isRanking ? 2 : 1,
    dataEnd: type === 'mvp_general' ? (isTop1 ? 1 : 10) : 1,
    source: type === 'mvp_day' && game ? { jogoId: game.id, jogoName: game.nome } : {},
    style: style as unknown as Record<string, unknown>,
  }
}
...
function mvpVisualWidth(block: PostArtworkBlock, style: PostArtworkMvpStyle) {
  ...
  if (style.layoutMode === 'table_only' || style.layoutMode === 'ranking_only') return style.tableWidth
  if (style.layoutMode === 'top1_only') return style.cardWidth
...
function mvpVisualHeight(block: PostArtworkBlock, style: PostArtworkMvpStyle) {
  ...
  if (style.layoutMode === 'top1_only') return style.cardHeight
  const count = Math.max(1, (block.dataEnd || 10) - (block.dataStart || 1) + 1)
  const tableCount = style.layoutMode === 'table_only' || style.layoutMode === 'ranking_only' ? count : Math.max(0, count - 1)
  const tableHeight = tableCount * style.tableRowHeight + Math.max(0, tableCount - 1) * style.tableRowGap
  return style.layoutMode === 'table_only' || style.layoutMode === 'ranking_only' ? tableHeight : Math.max(style.cardHeight, tableHeight)
}
...
async function drawMvpTop1(ctx: CanvasRenderingContext2D, player: PostArtworkPlayerRow | null, style: PostArtworkMvpStyle, x: number, y: number) {
  if (style.backgroundType === 'color') {
    ctx.fillStyle = style.backgroundColor
    ctx.fillRect(x, y, style.cardWidth, style.cardHeight)
  } else if (style.backgroundType === 'image' && style.backgroundUrl) {
  ...
  if (style.showPhoto && player.photo) {
    try {
      const photo = await loadImage(player.photo)
      const offscreen = document.createElement('canvas')
      offscreen.width = Math.max(1, Math.round(style.photoWidth))
      offscreen.height = Math.max(1, Math.round(style.photoHeight))
      const pctx = offscreen.getContext('2d')
      if (pctx) {
        pctx.imageSmoothingEnabled = true
        pctx.imageSmoothingQuality = 'high'
        pctx.drawImage(photo, 0, 0, offscreen.width, offscreen.height)
        if (style.photoFade !== 'none') {
          const strength = Math.max(.08, Math.min(.92, style.photoFadeStrength / 100))
          let gradient: CanvasGradient
          if (style.photoFade === 'top') gradient = pctx.createLinearGradient(0, 0, 0, offscreen.height)
          else if (style.photoFade === 'left') gradient = pctx.createLinearGradient(0, 0, offscreen.width, 0)
          else if (style.photoFade === 'right') gradient = pctx.createLinearGradient(offscreen.width, 0, 0, 0)
          else gradient = pctx.createLinearGradient(0, offscreen.height, 0, 0)
          gradient.addColorStop(0, 'rgba(0,0,0,0)')
          gradient.addColorStop(strength, 'rgba(0,0,0,1)')
        ...
        ctx.save()
        ctx.beginPath()
        ctx.roundRect(x + style.photoX, y + style.photoY, style.photoWidth, style.photoHeight, style.imageRadius)
        ctx.clip()
        ctx.drawImage(offscreen, x + style.photoX, y + style.photoY, style.photoWidth, style.photoHeight)
        ctx.restore()
      }
    } catch {}
  }

  const infoX = x + style.infoX
  const infoY = y + style.infoY
  if (style.backgroundType === 'color') {
    ctx.fillStyle = style.backgroundColor
    ctx.fillRect(infoX, infoY, style.infoWidth, style.infoHeight)
  } else if (style.backgroundType === 'image' && style.backgroundUrl) {
    await drawCover(ctx, style.backgroundUrl, infoX, infoY, style.infoWidth, style.infoHeight)
  }
  const centerX = infoX + style.infoWidth / 2
  let textY = infoY + Math.max(10, style.gap)
  ctx.textAlign = 'center'
  ctx.textBaseline = 'top'
  ctx.fillStyle = style.nameColor
  ctx.font = `${style.nameFontWeight} ${style.nameFontSize}px Arial`
  ctx.fillText(player.nick, centerX, textY, Math.max(30, style.infoWidth - style.gap * 2))
  textY += style.nameFontSize + Math.max(4, style.gap / 2)
  if (style.showTeam && player.team) {
    ctx.fillStyle = style.teamColor
    ctx.font = `700 ${style.teamFontSize}px Arial`
    ctx.fillText(player.team, centerX, textY, Math.max(30, style.infoWidth - style.gap * 2))
    textY += style.teamFontSize + Math.max(6, style.gap / 2)
  }
  const stats = [style.showKills ? `${player.kills} ABT` : '', style.showDrops ? `${player.drops} QD` : ''].filter(Boolean).join('   •   ')
  if (stats) {
    ctx.fillStyle = style.statsColor
    ctx.font = `800 ${style.statsFontSize}px Arial`
    ctx.fillText(stats, centerX, textY, Math.max(30, style.infoWidth - style.gap * 2))
  }
}
...
async function renderArtworkCanvas(project: PostArtworkProject, generalRows: PostArtworkTeamRow[], dayRows: Record<string, PostArtworkTeamRow[]>, mvpGeneralRows: PostArtworkPlayerRow[], mvpDayRows: Record<string, PostArtworkPlayerRow[]>, booyahRows: Record<string, PostArtworkBooyahRow[]> = {}, killLeaderRows: Record<string, PostArtworkPlayerRow[]> = {}, renderScale = 1, exportCaption = '', exportCaptionColor = '#FFFFFF') {
  ...
  for (const block of project.blocks.filter((item) => item.visible && item.type === 'booyahs_day')) {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      if (row.mapImage) {
        await drawCover(ctx, row.mapImage, x, y, cardWidth, style.cardHeight * .55)
        ctx.fillStyle = 'rgba(0,0,0,.34)'
        ctx.fillRect(x, y, cardWidth, style.cardHeight * .55)
      }
      if (row.logo) {
        try {
          const logo = await loadImage(row.logo)
      ...
      const accentY = y + style.cardHeight * .55
      ctx.fillStyle = style.accentColor
      ctx.fillRect(x, accentY, cardWidth, style.cardHeight * .2)
      ctx.fillStyle = style.textColor
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle'
      ctx.font = `900 ${style.teamFontSize}px Arial`
      ctx.fillText(row.name || 'Sem vencedor', x + cardWidth / 2, accentY + style.cardHeight * .1, Math.max(30, cardWidth - 14))
      ctx.font = `800 ${style.mapFontSize}px Arial`
      ctx.fillText(`${row.mapName} · ${row.round}`, x + cardWidth / 2, y + style.cardHeight * .8, Math.max(30, cardWidth - 14))
      ctx.font = `900 ${style.statsFontSize}px Arial`
      ctx.textAlign = 'left'; ctx.fillText(`${row.points} PTS`, x + 10, y + style.cardHeight - 18, Math.max(20, cardWidth * .48 - 12))
      ctx.textAlign = 'right'; ctx.fillText(`${row.kills} ABT`, x + cardWidth - 10, y + style.cardHeight - 18, Math.max(20, cardWidth * .48 - 12))
    }
  ...
  for (const block of project.blocks.filter((item) => item.visible && (item.type === 'mvp_general' || item.type === 'mvp_day' || item.type === 'kills_leaders'))) {
    ...
    if (block.type === 'mvp_general') {
      if (style.layoutMode === 'table_only' || style.layoutMode === 'ranking_only') {
        await drawMvpTable(ctx, rows, style, block.x, block.y)
      } else if (style.layoutMode === 'top1_only') {
        await drawMvpTop1(ctx, rows[0] || null, style, block.x, block.y)
      } else {
...
export function PostArtworkWorkspace({ campeonatoId, mode = 'edit', initialArtworkId }: { campeonatoId: string; mode?: 'edit' | 'generate' | 'manage' | 'library'; initialArtworkId?: string }) {
  ...
  function addMvpTop1() {
    if (!draft) return
    const block = createMvpBlock('mvp_general', draft.blocks.filter((item) => item.type === 'mvp_general').length, undefined, 'top1_only')
    setDraft({ ...draft, blocks: [...draft.blocks, block] })
    setSelectedBlockId(block.id)
  }

  function addMvpRanking() {
    if (!draft) return
    const block = createMvpBlock('mvp_general', draft.blocks.filter((item) => item.type === 'mvp_general').length, undefined, 'ranking_only')
    setDraft({ ...draft, blocks: [...draft.blocks, block] })
    setSelectedBlockId(block.id)
  }
        ...
        {draft ? <>
          <section className="post-artworks-controls">
            ...
            {!selectedBlock ? <>
              ...
              <div className="post-artworks-grid2"><label>X<EditableNumberInput value={selectedBlock.x} min={-20000} max={20000} onCommit={(value) => patchBlock(selectedBlock.id, { x: value })} /></label><label>Y<EditableNumberInput value={selectedBlock.y} min={-20000} max={20000} onCommit={(value) => patchBlock(selectedBlock.id, { y: value })} /></label>{selectedBlock.type === 'qualified_teams' ? <label>Top que classifica<input value={selectedBlock.dataEnd && selectedBlock.dataEnd > 0 ? `Top ${selectedBlock.dataEnd}` : 'Definido pelo jogo'} readOnly /></label> : selectedBlock.type === 'mvp_general' && selectedMvpStyle?.layoutMode === 'top1_only' ? <label>Posição<input value="Top 1" readOnly /></label> : selectedBlock.type === 'mvp_general' ? <><label>Do ranking<EditableNumberInput value={selectedBlock.dataStart || 2} min={1} max={50} onCommit={(value) => patchBlock(selectedBlock.id, { dataStart: value, dataEnd: Math.max(value, selectedBlock.dataEnd || 10) })} /></label><label>Até posição<EditableNumberInput value={selectedBlock.dataEnd || 10} min={selectedBlock.dataStart || 2} max={50} onCommit={(value) => patchBlock(selectedBlock.id, { dataEnd: value })} /></label></> : selectedMvpStyle ? <label>Posição no ranking<EditableNumberInput value={selectedBlock.dataStart || 1} min={1} max={999} onCommit={(value) => patchBlock(selectedBlock.id, { dataStart: value, dataEnd: value })} /></label> : <><label>Do item<EditableNumberInput value={selectedBlock.dataStart || 1} min={1} max={999} onCommit={(value) => patchBlock(selectedBlock.id, { dataStart: value, dataEnd: Math.max(value, selectedBlock.dataEnd || value) })} /></label><label>Até<EditableNumberInput value={selectedBlock.dataEnd || 12} min={selectedBlock.dataStart || 1} max={999} onCommit={(value) => patchBlock(selectedBlock.id, { dataEnd: value })} /></label></>}</div>
              {selectedBooyahStyle ? <><div className="post-artworks-subtitle"><strong>Cards de Booyah</strong><small>Um card por queda. A largura de cada card muda automaticamente conforme a quantidade de quedas, mantendo a largura geral definida.</small></div><div className="post-artworks-grid2"><label>Altura dos cards<EditableNumberInput value={selectedBooyahStyle.cardHeight} min={100} max={1200} onCommit={(value) => patchBooyahStyle({ cardHeight: value })} /></label><label>Largura geral<EditableNumberInput value={selectedBooyahStyle.totalWidth} min={180} max={5000} onCommit={(value) => patchBooyahStyle({ totalWidth: value })} /></label><label>Espaço entre cards<EditableNumberInput value={selectedBooyahStyle.gap} min={0} max={120} onCommit={(value) => patchBooyahStyle({ gap: value })} /></label><label>Tamanho da logo (%)<EditableNumberInput value={Math.round(selectedBooyahStyle.logoScale * 100)} min={15} max={80} onCommit={(value) => patchBooyahStyle({ logoScale: value / 100 })} /></label><label>Fonte equipe<EditableNumberInput value={selectedBooyahStyle.teamFontSize} min={8} max={120} onCommit={(value) => patchBooyahStyle({ teamFontSize: value })} /></label><label>Fonte mapa<EditableNumberInput value={selectedBooyahStyle.mapFontSize} min={8} max={100} onCommit={(value) => patchBooyahStyle({ mapFontSize: value })} /></label><label>Fonte estatísticas<EditableNumberInput value={selectedBooyahStyle.statsFontSize} min={8} max={100} onCommit={(value) => patchBooyahStyle({ statsFontSize: value })} /></label></div><PaletteColorField label="Fundo do card" value={selectedBooyahStyle.backgroundColor} palette={currentPalette} onChange={(value) => patchBooyahStyle({ backgroundColor: value })} /><PaletteColorField label="Cor de destaque" value={selectedBooyahStyle.accentColor} palette={currentPalette} onChange={(value) => patchBooyahStyle({ accentColor: value })} /><PaletteColorField label="Cor do texto" value={selectedBooyahStyle.textColor} palette={currentPalette} onChange={(value) => patchBooyahStyle({ textColor: value })} /></> : null}
              ...
              {selectedMvpStyle ? <>
                <div className="post-artworks-subtitle"><strong>{selectedBlock.type === 'kills_leaders' ? 'Card de líder de abates' : selectedBlock.type === 'mvp_general' ? (selectedMvpStyle.layoutMode === 'top1_only' ? 'MVP Geral · Top 1' : selectedMvpStyle.layoutMode === 'ranking_only' ? 'MVP Geral · Top 2+' : 'MVP Geral') : 'Card MVP'}</strong><small>{selectedBlock.type === 'mvp_general' ? 'Top 1 e ranking são blocos independentes e podem ficar em fatias diferentes.' : 'Um jogador por bloco, independente da transmissão.'}</small></div>
                {selectedBlock.type === 'mvp_general' ? <label>Tipo do bloco<select value={selectedMvpStyle.layoutMode} onChange={(event) => patchMvpStyle({ layoutMode: event.target.value as PostArtworkMvpStyle['layoutMode'] })}><option value="top1_only">Somente Top 1</option><option value="ranking_only">Somente ranking</option><option value="card_table">Legado: Top 1 + ranking</option><option value="table_only">Legado: tabela completa</option></select></label> : null}
                {(selectedBlock.type !== 'mvp_general' || selectedMvpStyle.layoutMode === 'top1_only' || selectedMvpStyle.layoutMode === 'card_table') ? <>
                  <div className="post-artworks-grid2"><label>Largura da área<EditableNumberInput value={selectedMvpStyle.cardWidth} min={180} max={2400} onCommit={(value) => patchMvpStyle({ cardWidth: value })} /></label><label>Altura da área<EditableNumberInput value={selectedMvpStyle.cardHeight} min={180} max={2400} onCommit={(value) => patchMvpStyle({ cardHeight: value })} /></label><label>Espaçamento<EditableNumberInput value={selectedMvpStyle.gap} min={0} max={120} onCommit={(value) => patchMvpStyle({ gap: value })} /></label></div>
                  <div className="post-artworks-mvp-checks"><label className="post-artworks-check"><input type="checkbox" checked={selectedMvpStyle.showPhoto} onChange={(event) => patchMvpStyle({ showPhoto: event.target.checked })} /> Foto</label><label className="post-artworks-check"><input type="checkbox" checked={selectedMvpStyle.showTeam} onChange={(event) => patchMvpStyle({ showTeam: event.target.checked })} /> Equipe</label><label className="post-artworks-check"><input type="checkbox" checked={selectedMvpStyle.showKills} onChange={(event) => patchMvpStyle({ showKills: event.target.checked })} /> Abates</label><label className="post-artworks-check"><input type="checkbox" checked={selectedMvpStyle.showDrops} onChange={(event) => patchMvpStyle({ showDrops: event.target.checked })} /> Quedas</label></div>
                  {selectedBlock.type === 'mvp_general' && selectedMvpStyle.layoutMode === 'top1_only' ? <div className="post-artworks-mvp-free-controls">
                    <div className="post-artworks-subtitle"><strong>Foto livre</strong><small>Posicione e dimensione a foto sem prender ao card de informações.</small></div>
                    <div className="post-artworks-grid2"><label>Foto X<EditableNumberInput value={selectedMvpStyle.photoX} min={-2400} max={2400} onCommit={(value) => patchMvpStyle({ photoX: value })} /></label><label>Foto Y<EditableNumberInput value={selectedMvpStyle.photoY} min={-2400} max={2400} onCommit={(value) => patchMvpStyle({ photoY: value })} /></label><label>Largura foto<EditableNumberInput value={selectedMvpStyle.photoWidth} min={20} max={2400} onCommit={(value) => patchMvpStyle({ photoWidth: value })} /></label><label>Altura foto<EditableNumberInput value={selectedMvpStyle.photoHeight} min={20} max={2400} onCommit={(value) => patchMvpStyle({ photoHeight: value })} /></label></div>
                    <label>Degradê da foto<select value={selectedMvpStyle.photoFade} onChange={(event) => patchMvpStyle({ photoFade: event.target.value as PostArtworkMvpStyle['photoFade'] })}><option value="none">Sem degradê</option><option value="bottom">Sumir embaixo</option><option value="top">Sumir em cima</option><option value="left">Sumir à esquerda</option><option value="right">Sumir à direita</option></select></label>
                    {selectedMvpStyle.photoFade !== 'none' ? <label>Força do degradê<EditableNumberInput value={selectedMvpStyle.photoFadeStrength} min={8} max={92} onCommit={(value) => patchMvpStyle({ photoFadeStrength: value })} /></label> : null}
                    <div className="post-artworks-subtitle"><strong>Card de informações livre</strong><small>Controle posição e tamanho independentemente da foto.</small></div>
                    <div className="post-artworks-grid2"><label>Info X<EditableNumberInput value={selectedMvpStyle.infoX} min={-2400} max={2400} onCommit={(value) => patchMvpStyle({ infoX: value })} /></label><label>Info Y<EditableNumberInput value={selectedMvpStyle.infoY} min={-2400} max={2400} onCommit={(value) => patchMvpStyle({ infoY: value })} /></label><label>Largura info<EditableNumberInput value={selectedMvpStyle.infoWidth} min={80} max={2400} onCommit={(value) => patchMvpStyle({ infoWidth: value })} /></label><label>Altura info<EditableNumberInput value={selectedMvpStyle.infoHeight} min={60} max={1200} onCommit={(value) => patchMvpStyle({ infoHeight: value })} /></label></div>
                  </div> : null}
                </> : null}
                {selectedBlock.type === 'mvp_general' && selectedMvpStyle.layoutMode !== 'top1_only' ? <><div className="post-artworks-subtitle"><strong>Tabela do ranking</strong><small>{selectedMvpStyle.layoutMode === 'ranking_only' ? `Exibe do Top ${selectedBlock.dataStart || 2} até o Top ${selectedBlock.dataEnd || 10}.` : 'Configuração da lista de jogadores.'}</small></div><div className="post-artworks-grid2"><label>Largura da tabela<EditableNumberInput value={selectedMvpStyle.tableWidth} min={260} max={2400} onCommit={(value) => patchMvpStyle({ tableWidth: value })} /></label><label>Altura da linha<EditableNumberInput value={selectedMvpStyle.tableRowHeight} min={24} max={180} onCommit={(value) => patchMvpStyle({ tableRowHeight: value })} /></label><label>Espaço entre linhas<EditableNumberInput value={selectedMvpStyle.tableRowGap} min={0} max={80} onCommit={(value) => patchMvpStyle({ tableRowGap: value })} /></label><label>Tamanho do texto<EditableNumberInput value={selectedMvpStyle.tableFontSize} min={8} max={80} onCommit={(value) => patchMvpStyle({ tableFontSize: value })} /></label><PaletteColorField label="Cor do texto" value={selectedMvpStyle.tableTextColor} palette={currentPalette} onChange={(value) => patchMvpStyle({ tableTextColor: value })} /></div><label>Fundo das linhas<select value={selectedMvpStyle.tableBackgroundType} onChange={(event) => patchMvpStyle({ tableBackgroundType: event.target.value as 'color' | 'image' | 'none' })}><option value="color">Cor</option><option value="image">Imagem</option><option value="none">Sem fundo</option></select></label>{selectedMvpStyle.tableBackgroundType === 'color' ? <PaletteColorField label="Cor das linhas" value={selectedMvpStyle.tableBackgroundColor} palette={currentPalette} onChange={(value) => patchMvpStyle({ tableBackgroundColor: value })} /> : null}</> : null}
              </> : null}
            </>}
              ...
              <div className="post-artworks-preview" style={{ width: draft.width * previewScale, height: draft.height * previewScale, backgroundColor: draft.background_color, backgroundImage: draft.background_url ? `url(${JSON.stringify(draft.background_url)})` : undefined }}>
                ...
                {draft.blocks.filter((block) => block.visible && block.type === 'booyahs_day').map((rawBlock) => {
                  ...
                  return <div key={block.id} className={`post-artworks-booyah-block${block.id === selectedBlockId ? ' active' : ''}`} style={{ left: block.x * previewScale, top: block.y * previewScale, width: style.totalWidth * previewScale, height: style.cardHeight * previewScale, gap: style.gap * previewScale }} onPointerDown={(event) => beginDrag(event, block)} onPointerMove={drag} onPointerUp={endDrag} onPointerCancel={endDrag}>
                    {rows.map((row) => <div key={row.partidaId} className="post-artworks-booyah-card" style={{ width: cardWidth * previewScale, height: style.cardHeight * previewScale, backgroundColor: style.backgroundColor, color: style.textColor }}><div className="post-artworks-booyah-media" style={{ height: style.cardHeight * .55 * previewScale, backgroundImage: row.mapImage ? `linear-gradient(rgba(0,0,0,.34),rgba(0,0,0,.34)),url(${JSON.stringify(row.mapImage)})` : undefined }}>{row.logo ? <img src={row.logo} alt="" draggable={false} style={{ maxWidth: cardWidth * style.logoScale * previewScale, maxHeight: style.cardHeight * .34 * previewScale }} /> : null}</div><strong style={{ height: style.cardHeight * .2 * previewScale, background: style.accentColor, fontSize: Math.max(7, style.teamFontSize * previewScale) }}>{row.name}</strong><small style={{ fontSize: Math.max(6, style.mapFontSize * previewScale) }}>{row.mapName} · {row.round}</small><div className="post-artworks-booyah-stats" style={{ fontSize: Math.max(6, style.statsFontSize * previewScale) }}><b>{row.points} PTS</b><b>{row.kills} ABT</b></div></div>)}
                ...
                {draft.blocks.filter((block) => block.visible && (block.type === 'mvp_general' || block.type === 'mvp_day' || block.type === 'kills_leaders')).map((rawBlock) => {
                  ...
                  if (block.type === 'mvp_general') {
                    const photoMask = style.photoFade === 'none' ? undefined : style.photoFade === 'bottom' ? `linear-gradient(to top, transparent 0%, black ${style.photoFadeStrength}%, black 100%)` : style.photoFade === 'top' ? `linear-gradient(to bottom, transparent 0%, black ${style.photoFadeStrength}%, black 100%)` : style.photoFade === 'left' ? `linear-gradient(to right, transparent 0%, black ${style.photoFadeStrength}%, black 100%)` : `linear-gradient(to left, transparent 0%, black ${style.photoFadeStrength}%, black 100%)`
                    const top1 = rows[0] || null
                    const freeTop1 = <div className="post-artworks-mvp-top1-free" style={{ position: 'relative', width: style.cardWidth * previewScale, height: style.cardHeight * previewScale, backgroundColor: style.backgroundType === 'color' ? style.backgroundColor : 'transparent', backgroundImage: style.backgroundType === 'image' && style.backgroundUrl ? `url(${JSON.stringify(style.backgroundUrl)})` : undefined, backgroundSize: 'cover', backgroundPosition: 'center' }}>{top1 ? <>{style.showPhoto ? <div className="post-artworks-mvp-photo-free" style={{ left: style.photoX * previewScale, top: style.photoY * previewScale, width: style.photoWidth * previewScale, height: style.photoHeight * previewScale, borderRadius: style.imageRadius * previewScale, maskImage: photoMask, WebkitMaskImage: photoMask }}>{top1.photo ? <img src={top1.photo} alt="" draggable={false} /> : <span>{top1.nick.slice(0, 1)}</span>}</div> : null}<div className="post-artworks-mvp-info-free" style={{ left: style.infoX * previewScale, top: style.infoY * previewScale, width: style.infoWidth * previewScale, height: style.infoHeight * previewScale, gap: Math.max(2, style.gap * .45 * previewScale), backgroundColor: style.backgroundType === 'color' ? style.backgroundColor : 'transparent', backgroundImage: style.backgroundType === 'image' && style.backgroundUrl ? `url(${JSON.stringify(style.backgroundUrl)})` : undefined }}><strong style={{ color: style.nameColor, fontSize: Math.max(8, style.nameFontSize * previewScale), fontWeight: style.nameFontWeight }}>{top1.nick}</strong>{style.showTeam && top1.team ? <small style={{ color: style.teamColor, fontSize: Math.max(7, style.teamFontSize * previewScale) }}>{top1.team}</small> : null}<div className="post-artworks-mvp-stats" style={{ color: style.statsColor, fontSize: Math.max(7, style.statsFontSize * previewScale) }}>{style.showKills ? <b>{top1.kills}<small>ABATES</small></b> : null}{style.showDrops ? <b>{top1.drops}<small>QUEDAS</small></b> : null}</div></div></> : null}</div>
                    return <div key={block.id} className={`post-artworks-mvp-block post-artworks-mvp-general${block.id === selectedBlockId ? ' active' : ''}`} style={{ left: block.x * previewScale, top: block.y * previewScale, width: mvpVisualWidth(block, style) * previewScale, height: mvpVisualHeight(block, style) * previewScale, gap: style.gap * previewScale }} onPointerDown={(event) => beginDrag(event, block)} onPointerMove={drag} onPointerUp={endDrag} onPointerCancel={endDrag}>{style.layoutMode === 'top1_only' ? freeTop1 : style.layoutMode === 'table_only' || style.layoutMode === 'ranking_only' ? table(rows) : <>{card(rows[0] || null)}{rows.length > 1 ? table(rows.slice(1)) : null}</>}{!rows.length ? <div className="post-artworks-no-data">Sem dados de MVP</div> : null}</div>
                  }
