...
type PackageInspectorItem = StreamPackageAssetKey | `column_${StreamTableColumnStyleKey}` | 'event_title' | 'table_row' | 'table_header' | 'table_block' | 'card_block'
          ...
          <div className="stream-package-editor-grid">
              ...
              {activePanel === 'assets' ? (
                <section className="stream-package-section stream-package-control-card stream-package-properties-card">
                  {selectedInspectorItem === 'table_row' ? (
                    ...
                    <><div className="stream-package-section-title"><div><small>Elemento de tabela</small><strong>Legenda da tabela</strong><p>Configura os títulos como QD, B!, ABT e PTS.</p></div></div><div className="stream-package-property-group"><b>Legenda</b><label className="stream-package-switch-row"><span><b>Exibir legenda</b></span><input type="checkbox" checked={pack.shared_config.table.showHeaders} onChange={(e) => patchTable({ showHeaders: e.target.checked })} /></label><label>Altura<input type="number" min={24} max={100} value={pack.shared_config.table.headerHeight} onChange={(e) => patchTable({ headerHeight: Number(e.target.value) || 38 })} /></label></div></>
            ...
            <aside className="stream-package-elements">
              ...
              <div className="stream-package-element-list">
                ...
                {activeMeta.structure === 'table' ? <div className="stream-package-element-group"><small>Tabela</small><button type="button" className={selectedInspectorItem === 'table_row' ? 'active' : ''} onClick={() => { setSelectedInspectorItem('table_row'); setActivePanel('assets') }}><span>Linha</span><b>Linha</b></button><button type="button" className={selectedInspectorItem === 'table_header' ? 'active' : ''} onClick={() => { setSelectedInspectorItem('table_header'); setActivePanel('assets') }}><span>Legenda</span><b>Legenda da tabela</b></button><small>Colunas de {activeMeta.name}</small>{(activeConfig.columns || []).map((styleKey) => <button type="button" key={styleKey} className={selectedInspectorItem === `column_${styleKey}` ? 'active' : ''} onClick={() => { setSelectedInspectorItem(`column_${styleKey}` as PackageInspectorItem); setActivePanel('assets') }}><span>Coluna</span><b>{STREAM_OVERLAY_COLUMN_META[styleKey]?.label || styleKey}</b></button>)}</div> : null}
              </div>
            </aside>
          </div>
        </main>
