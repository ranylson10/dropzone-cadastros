          ...
          <div className="stream-package-editor-grid">
                ...
                <section className="stream-package-section stream-package-control-card stream-package-properties-card">
                  {selectedInspectorItem === 'table_row' ? (
                    ...
                    <>
                      ...
                      <div className="stream-package-property-group"><b>Texto</b><label>Fonte<select value={selectedColumnStyle.fontFamily} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { fontFamily: e.target.value })}>{FONT_OPTIONS.map((font) => <option key={font} value={font}>{font}</option>)}</select></label><div className="stream-package-quad-grid"><label>Tamanho<input type="number" min={8} max={160} value={selectedColumnStyle.fontSize} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { fontSize: Number(e.target.value) || 8 })} /></label><label>Peso<input type="number" min={100} max={900} step={100} value={selectedColumnStyle.fontWeight} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { fontWeight: Number(e.target.value) || 400 })} /></label><label>Cor<input type="color" value={selectedColumnStyle.color} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { color: e.target.value })} /></label><label>Inclinação<select value={selectedColumnStyle.fontStyle} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { fontStyle: e.target.value as 'normal' | 'italic' })}><option value="normal">Reta</option><option value="italic">Itálica</option></select></label><label>Horizontal<select value={selectedColumnStyle.align} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { align: e.target.value as 'left' | 'center' | 'right' })}><option value="left">Esquerda</option><option value="center">Centro</option><option value="right">Direita</option></select></label><label>Vertical<select value={selectedColumnStyle.verticalAlign || 'center'} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { verticalAlign: e.target.value as 'top' | 'center' | 'bottom' })}><option value="top">Topo</option><option value="center">Centro</option><option value="bottom">Base</option></select></label><label>Desloc. X<input type="number" value={selectedColumnStyle.offsetX ?? 0} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { offsetX: Number(e.target.value) || 0 })} /></label><label>Desloc. Y<input type="number" value={selectedColumnStyle.offsetY ?? 0} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { offsetY: Number(e.target.value) || 0 })} /></label></div></div>
                      <div className="stream-package-property-group"><b>Célula</b><label>Largura (vazio = automática)<input type="number" min={20} value={selectedColumnStyle.width || ''} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { width: e.target.value ? Number(e.target.value) : null })} /></label><label>Preenchimento<select value={selectedColumnStyle.backgroundType} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { backgroundType: e.target.value as 'solid' | 'gradient' | 'image' })}><option value="solid">Cor sólida</option><option value="gradient">Degradê</option><option value="image">Imagem</option></select></label>{selectedColumnStyle.backgroundType === 'solid' ? <label>Cor<input type="color" value={selectedColumnStyle.backgroundColor} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { backgroundColor: e.target.value })} /></label> : null}{selectedColumnStyle.backgroundType === 'gradient' ? <label>Degradê CSS<input value={selectedColumnStyle.backgroundGradient} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { backgroundGradient: e.target.value })} /></label> : null}{selectedColumnStyle.backgroundType === 'image' && selectedColumnStyle.assetKey ? <><div className="stream-package-selected-asset">{pack.assets[selectedColumnStyle.assetKey] ? <img src={pack.assets[selectedColumnStyle.assetKey]} alt="" /> : <span>Sem imagem</span>}<div><b>{PACKAGE_ASSETS.find((asset) => asset.key === selectedColumnStyle.assetKey)?.label || 'Fundo da coluna'}</b><small>Este é o único fundo usado por esta coluna.</small></div></div><label className="stream-secondary-btn stream-package-inspector-upload">{uploading === selectedColumnStyle.assetKey ? <Loader2 className="spin" size={14} /> : <ImagePlus size={14} />} Trocar imagem desta coluna<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(e) => void uploadAsset(selectedColumnStyle.assetKey!, e.target.files?.[0])} /></label></> : null}<div className="stream-package-quad-grid"><label>Cor da borda<input type="color" value={selectedColumnStyle.borderColor} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { borderColor: e.target.value })} /></label><label>Espessura<input type="number" min={0} max={30} value={selectedColumnStyle.borderWidth} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { borderWidth: Number(e.target.value) || 0 })} /></label><label>Canto<input type="number" min={0} max={100} value={selectedColumnStyle.borderRadius} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { borderRadius: Number(e.target.value) || 0 })} /></label><label>Opacidade (%)<input type="number" min={0} max={100} value={Math.round((selectedColumnStyle.opacity ?? 1) * 100)} onChange={(e) => patchColumnStyle(selectedColumnStyleKey, { opacity: Math.max(0, Math.min(1, (Number(e.target.value) || 0) / 100)) })} /></label></div></div>
                    ...
                    <>
                      ...
                      <label>Fonte<select value={pack.shared_config.identity.fontFamily} onChange={(e) => patchIdentity({ fontFamily: e.target.value })}>{FONT_OPTIONS.map((font) => <option key={font} value={font}>{font}</option>)}</select></label>
            ...
            <aside className="stream-package-elements">
              ...
              <div className="stream-package-element-list">
                {activeLooseImage.show ? <div className="stream-package-item-row"><button type="button" className={selectedInspectorItem === 'event_logo' ? 'active' : ''} onClick={() => { setSelectedInspectorItem('event_logo'); setActivePanel('assets') }}><span>Imagem</span><b>Logo do campeonato</b></button><button type="button" className="stream-package-element-remove" title="Remover logo" aria-label="Remover logo" onClick={() => patchLooseImage({ show: false })}><Trash2 size={13} /></button></div> : null}
                {activeLooseText.show ? <div className="stream-package-item-row"><button type="button" className={selectedInspectorItem === 'event_title' ? 'active' : ''} onClick={() => { setSelectedInspectorItem('event_title'); setActivePanel('assets') }}><span>Texto</span><b>Texto livre</b></button><button type="button" className="stream-package-element-remove" title="Remover texto" aria-label="Remover texto" onClick={() => patchLooseText({ show: false })}><Trash2 size={13} /></button></div> : null}
                <div className="stream-package-element-group"><small>Itens</small><div className="stream-package-add-items"><button type="button" title="Adicionar texto" aria-label="Adicionar texto" onClick={() => addSceneItem('text')}><Type size={14} /></button><button type="button" title="Adicionar imagem" aria-label="Adicionar imagem" onClick={() => addSceneItem('image')}><ImageIcon size={14} /></button><button type="button" title="Adicionar cronômetro" aria-label="Adicionar cronômetro" onClick={() => addSceneItem('timer')}><Timer size={14} /></button><button type="button" title="Adicionar contador de quedas" aria-label="Adicionar contador de quedas" onClick={() => addSceneItem('round_counter')}>#</button></div>{(activeConfig.sceneItems || []).map((item) => <div className="stream-package-item-row" key={item.id}><button type="button" className={selectedInspectorItem === `scene_${item.id}` ? 'active' : ''} onClick={() => { setSelectedInspectorItem(`scene_${item.id}`); setActivePanel('assets') }}><span>{item.type === 'image' ? 'Imagem' : item.type === 'round_counter' ? 'Quedas' : item.type === 'timer' ? 'Timer' : 'Texto'}</span><b>{item.type === 'text' || item.type === 'timer' ? (item.text || 'Texto livre') : item.type === 'image' ? 'Imagem livre' : 'Contador de quedas'}</b></button><button type="button" className="stream-package-element-remove" title="Remover item" aria-label="Remover item" onClick={() => removeSceneItem(item.id)}><Trash2 size={13} /></button></div>)}</div>
