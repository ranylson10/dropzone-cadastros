...
export function JogadorPanel(props: {
  ...
  const enrolledChampionships = useMemo(() => {
    ...
    for (const registration of props.registrations) {
      const championshipId = String(registration.parent_id || registration.data?.championship_id || '')
      const teamId = String(registration.ref_id || registration.data?.team_id || '')
      const key = `${championshipId}:${teamId || registration.id}`
      if (!byRegistration.has(key)) byRegistration.set(key, registration)
    }
    return [...byRegistration.values()].map((registration) => {
      const championshipId = String(registration.parent_id || registration.data?.championship_id || '')
      const teamId = String(registration.ref_id || registration.data?.team_id || '')
      const championship = props.championships.find((item) => item.id === championshipId)
      const team = props.teams.find((item) => item.id === teamId)
      return {
        registration,
        championshipName: championship ? rowTitle(championship) : String(registration.data?.championship_name || 'Campeonato não identificado'),
        championshipType: dataText(championship, 'tipo') || '',
        teamName: team ? rowTitle(team) : String(registration.data?.team_name || 'Equipe não identificada'),
        teamTag: dataText(team, 'tag') || String(registration.data?.team_tag || ''),
      }
    })
  }, [props.registrations, props.championships, props.teams])

  function openLineupToken() {
    const raw = lineupToken.trim()
    if (!raw) {
      setLineupTokenError('Cole o token ou link de escalação recebido.')
      return
    }
    const match = raw.match(/\/escala\/([^/?#]+)/i)
    const token = (match?.[1] || raw).replace(/\s/g, '')
    if (!token || /^https?:\/\//i.test(token)) {
      setLineupTokenError('Use o link completo de escalação ou somente o token recebido.')
      return
    }
    window.location.assign(`/escala/${encodeURIComponent(decodeURIComponent(token))}`)
  }
