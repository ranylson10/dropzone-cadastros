    ...
    <div className="dashboard player-dashboard">
      <section className="panel span-3">
        ...
        <div className="producer-tabs manager-champ-tabs" style={{ marginBottom: 12 }}>
          <button type="button" className={tab === 'resumo' ? 'active' : ''} onClick={() => setTab('resumo')}>
            Resumo
          </button>
        ...
        {tab === 'perfil' ? (
          <ProfileEditForm
            ...
            initial={{
              ...
              avatar_url: dataText(props.account, 'avatar_url') || dataText(props.account, 'foto_url'),
              bio: dataText(props.account, 'bio'),
              id_jogo: dataText(props.account, 'id_jogo'),
              funcao: dataText(props.account, 'funcao'),
            }}
          />
        ) : (
          <div className="player-summary-grid">
            ...
            <div>
              <Users size={18} />
              <strong>{myLines.length}</strong>
              <span>Lines</span>
            </div>
            <div>
              <Swords size={18} />
              <strong>{props.registrations.length}</strong>
              <span>Campeonatos</span>
            </div>
            <div>
              <BarChart3 size={18} />
              <strong>0</strong>
              <span>Partidas pontuadas</span>
            </div>
          </div>
        )}
      </section>
      {tab === 'resumo' ? (
        <>
          <section className="panel span-2">
            ...
            <div className="cards">
              {props.registrations.length === 0 ? (
                <p className="empty">Você ainda não está inscrito em campeonato.</p>
              ) : null}
          ...
          <section className="panel">
            <h2>Minha equipe</h2>
            {myTeams.length === 0 ? <p className="empty">Você ainda não faz parte de uma equipe.</p> : null}
            <div className="team-line-grid">
              {myTeams.map((team) => (
                <article className="team-line-card" key={team.id}>
                  <img src={dataText(team, 'logo_url') || '/favicon.ico'} alt="" />
                  <div>
                    <strong>{rowTitle(team)}</strong>
                    <span>{dataText(team, 'tag') || 'Sem tag'}</span>
                  </div>
                </article>
              ))}
            </div>
          ...
          <section className="panel">
            <h2>Minhas lines</h2>
            {myLines.length === 0 ? <p className="empty">Nenhuma line vinculada ao seu elenco.</p> : null}
            <div className="team-line-grid">
              {myLines.map((line) => (
                <article className="team-line-card" key={line.id}>
                  <img src={dataText(line, 'logo_url') || '/favicon.ico'} alt="" />
                  <div>
