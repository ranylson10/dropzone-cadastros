...
export function ManagerPanel({ account }: { account: DropZoneRow }) {
  ...
  return <div className="dashboard manager-dashboard">
    <section className="panel span-3"><div className="section-head"><div><p className="eyebrow">Manager</p><h2>Minha página de vendas</h2></div><ShieldCheck /></div><div className="player-summary-grid"><div><Users size={18}/><strong>{sellerItems.length}</strong><span>Campeonatos</span></div><div><ShieldCheck size={18}/><strong>{sellerItems.filter((item) => item.status === 'ativo').length}</strong><span>Ativos</span></div><div><ShieldCheck size={18}/><strong>{sellerItems.filter((item) => item.status !== 'ativo').length}</strong><span>Pendentes / ocultos</span></div></div></section>
