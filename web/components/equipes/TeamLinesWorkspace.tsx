...
export function TeamLinesWorkspace({ equipeId }: { equipeId: string }) {
  ...
  return <section className="team-lines-site-workspace">
    ...
    {!selected
      ? <div className="team-lines-site-grid">{lines.map((line) => <button type="button" key={line.id} onClick={() => setSelected(line)}><span>{line.logo_url ? <img src={line.logo_url} alt=""/> : String(line.tag || line.nome || 'L').slice(0, 2)}</span><div><strong>{line.nome}</strong><small>{line.public_id ? `LN${line.public_id} · ` : ''}{line.campeonatos?.length || 0} campeonato(s)</small></div><ChevronRightIcon/></button>)}</div>
      : <LineRosterManager accessToken={token} equipeId={equipeId} line={selected} onBack={() => setSelected(null)} onChanged={() => void load(token)}/>
    }
  </section>
}
