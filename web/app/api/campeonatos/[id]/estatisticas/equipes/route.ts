...
import { listarEstatisticasEquipes, listarEstatisticasEquipesPublicadas } from '@backend/campeonatos/estatisticas/estatisticas.service'
...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const payload = imediato
      ? { equipes: await listarEstatisticasEquipes(id, filters), publicacao: { modo: 'imediato' } }
      : await listarEstatisticasEquipesPublicadas(id, filters)
