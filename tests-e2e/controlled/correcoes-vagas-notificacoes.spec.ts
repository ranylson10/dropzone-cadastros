...
test.describe('Correções operacionais — vagas e notificações', () => {
  test('protege leitura em massa e mantém capacidade coerente da fase de entrada', async ({ request, browser, baseURL }) => {
    ...
    const authorization = { Authorization: `Bearer ${produtoraToken}`, 'x-profile-type': 'produtora' }
