...
function accessTokenFromStorage(file: string, expectedOrigin: string): string {
  ...
  for (const entry of origin?.localStorage || []) {
    ...
    try {
      ...
      // Ignora chaves locais que não sejam a sessão do Supabase.
    }
  }
...
test.describe('Estrutura controlada — jogo e quedas com limpeza automática', () => {
  ...
  test('produtora cria fase, grupo, jogo, valida quedas, altera mapa e limpa tudo', async ({ request, baseURL }) => {
    test.skip(
