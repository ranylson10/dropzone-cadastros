...
test.describe('Commerce — carrinho e favoritos persistidos', () => {
  test('banco, APIs, web e app ficam vinculados por usuário', async () => {
    ...
    const purchasesApi = read('web/app/api/me/compras/vagas/route.ts')
    ...
    expect(checkoutApi).toContain('resolveBillingProfile')
    expect(checkoutApi).toContain('billing_profile?.document')
    expect(checkoutApi).toContain("method === 'pix'")
    ...
    expect(cartPage).toContain("/api/me/perfil-cobranca")
    expect(cartPage).toContain('billing_profile: !billingProfile')
    expect(cartPage).toContain('Dados de pagamento')
    expect(cartPage).toContain('className="cart-checkout"')
    expect(cartPage).toContain('RECOMENDADO PARA VOCÊ')
    expect(cartPage).toContain('MINHAS COMPRAS')
    expect(purchasesApi).toContain(".eq('auth_user_id', user.id)")
    expect(purchasesApi).toContain("['pago', 'liberado', 'consumido']")
