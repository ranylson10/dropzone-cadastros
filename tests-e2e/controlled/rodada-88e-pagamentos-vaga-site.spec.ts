...
test.describe('Rodada 88E — pagamento público de vaga', () => {
  test('site expõe meios seguros e preserva retorno para compra de vaga', async () => {
    ...
    expect(claimPage).toContain('vacancy-claim-steps')
    expect(claimPage).toContain('vacancy-guided-summary')
    expect(claimPage).toContain('vacancy-entry-review')
    expect(claimPage).toContain('Escalar jogadores')
    expect(claimPage).toContain("setLineId('__create__')")
    expect(claimPage).toContain('canConfirmEntry')
    ...
    expect(sellerSalesApi).toContain('paypalConfigured')
    expect(sellerSalesApi).toContain('metodo_sugerido: method')
    expect(purchase).toContain("if (method === 'paypal')")
    ...
    expect(sellerPanel).toContain("className={saleMethod === 'paypal' ? 'button small' : 'button small secondary'}")
    expect(sellerPanel).toContain("creatingSale ? 'Criando link...' : 'Gerar link de compra'")
    ...
    expect(css).toContain('.vacancy-claim-steps')
    expect(css).toContain('.vacancy-guided-summary')
    expect(css).toContain('.vacancy-entry-review')
  })
})
