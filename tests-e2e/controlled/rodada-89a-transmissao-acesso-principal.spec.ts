...
test('89A mantém Transmissão como aba principal do campeonato', () => {
  ...
  expect(panel).toContain("tab === 'stream' ? <CampeonatoStreamTab")
  expect(tabs).toContain("id: 'stream'")
  expect(tabs).toContain("label: 'Transmissão'")
})

test('89A não deixa Transmissão dependente de menu secundário', () => {
  const panel = read('web/features/dropzone/panels/produtora/ProdutoraPanel.tsx')
  expect(panel).toContain('{producerTabs')
  expect(panel).toContain('.map((item) => (')
  expect(panel).not.toContain('mainTabs.includes(item.id)')
})
