...
test.describe('Rodada 87O1 — agenda contextual somente com datas de jogos', () => {
  ...
  test('agenda contextual agrupa e mostra somente datas com eventos', () => {
    ...
    expect(calendar).toContain('agenda-event-date-strip')
    expect(calendar).toContain('eventDates.map')
    expect(calendar).toContain('Nenhum jogo ou compromisso agendado neste mês.')
    expect(css).toContain('.agenda-event-date-strip')
    expect(css).toContain('.agenda-event-date-panel')
  })
  ...
  test('campeonato e perfis públicos não permitem criar agenda', () => {
    ...
    expect(championship).toContain('Somente as datas que possuem jogos do campeonato')
  })
})
