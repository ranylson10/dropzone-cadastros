...
const readCompact = (file: string) => read(file).replace(/\s+/g, '').replace(/"/g, "'")
...
test.describe('Mobile — LEALT compacto', () => {
  ...
  test('propaga densidade compacta para perfis, token e painel interno', async () => {
    const create = readCompact('app/src/screens/ProfileCreateScreen.tsx')
    const profile = readCompact('app/src/screens/ProfileManagementScreen.tsx')
    const team = read('app/src/screens/TeamCreateScreen.tsx')
    const token = readCompact('app/src/screens/TokenActionScreen.tsx')
    const control = readCompact('app/src/screens/ControlPanelScreen.tsx')
    ...
    expect(token).toContain("title:{color:colors.ink,fontSize:20")
  ...
  test('propaga o LEALT compacto para operação, elenco, agenda e carteira', async () => {
    const championship = readCompact('app/src/screens/ChampionshipManagementScreen.tsx')
    const games = readCompact('app/src/screens/ChampionshipGamesPanel.tsx')
    const teams = readCompact('app/src/screens/ChampionshipTeamsPanel.tsx')
    const lines = readCompact('app/src/screens/LineManagementScreen.tsx')
    const roster = readCompact('app/src/screens/TeamRosterScreen.tsx')
    const agenda = readCompact('app/src/screens/AgendaScreen.tsx')
    const wallet = readCompact('app/src/screens/WalletScreen.tsx')
    ...
    expect(wallet).toContain("bankCard:{backgroundColor:colors.brandDark")
  ...
  test('fecha a varredura visual nas telas restantes do app', async () => {
    const calls = readCompact('app/src/screens/ChampionshipCallsPanel.tsx')
    const finalization = readCompact('app/src/screens/ChampionshipFinalizationPanel.tsx')
    const rulebook = readCompact('app/src/screens/ChampionshipRulebookPanel.tsx')
    const scorer = readCompact('app/src/screens/ChampionshipScorerPanel.tsx')
    const settings = readCompact('app/src/screens/ChampionshipSettingsPanel.tsx')
    const stream = readCompact('app/src/screens/ChampionshipStreamPanel.tsx')
    const search = readCompact('app/src/screens/GlobalSearchScreen.tsx')
    const invites = readCompact('app/src/screens/InvitesScreen.tsx')
    const lineup = readCompact('app/src/screens/LineupScreen.tsx')
    const players = readCompact('app/src/screens/TeamPlayersPanel.tsx')
    const staff = readCompact('app/src/screens/TeamStaffPanel.tsx')
    const commerce = readCompact('app/src/screens/CommerceScreen.tsx')
    const seller = readCompact('app/src/screens/SellerSalesScreen.tsx')
    const broadcast = readCompact('app/src/screens/BroadcastDeskPanel.tsx')
    const purchase = readCompact('app/src/screens/PurchaseClaimScreen.tsx')
    ...
    expect(search).toContain("heroTitle:{marginTop:3,color:colors.ink,fontSize:19")
    expect(invites).toContain("header:{minHeight:84")
    expect(lineup).toContain("heroTitle:{color:colors.ink,fontSize:24")
    expect(players).toContain("card:{padding:10,gap:8")
    expect(staff).toContain("loading:{minHeight:72")
    expect(commerce).toContain("summaryValue:{marginTop:4,color:colors.ink,fontSize:24")
    expect(seller).toContain("balanceValue:{marginTop:4,color:colors.ink,fontSize:22")
